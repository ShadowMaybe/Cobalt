...
{	
	...
	if(!IOGLES2) {
	  ...
	  struct Library *Warp3DNovaBase = NULL;
		...
		if(!Warp3DNovaBase) {
	        COBALT_LOGE("Error, cannot open Warp3DNova.library!\n");
	        return;
	    }
	    COBALT_LOGD("Using Warp3DNova.library v%d revision %d\n", Warp3DNovaBase->lib_Version, Warp3DNovaBase->lib_Revision);
		if (!(Warp3DNovaBase->lib_Version > MIN_W3DNOVA_LIB_VERSION || (Warp3DNovaBase->lib_Version == MIN_W3DNOVA_LIB_VERSION && Warp3DNovaBase->lib_Revision >= MIN_W3DNOVA_LIB_REVISION)))  {
	        COBALT_LOGE("Warning, your Warp3DNovaBase.library is too old, minimum is v%d.%d, please update!\n", MIN_W3DNOVA_LIB_VERSION,MIN_W3DNOVA_LIB_REVISION);
		}	
	  ...
	  if(!LOGLES2) {
	      COBALT_LOGE("Error, cannot open ogles2 Library!\n");
	      return;
	  }
	  COBALT_LOGD("Using OGLES2.library v%d revision %d\n", LOGLES2->lib_Version, LOGLES2->lib_Revision);
		if (!(LOGLES2->lib_Version > MIN_OGLES2_LIB_VERSION || (LOGLES2->lib_Version == MIN_OGLES2_LIB_VERSION && LOGLES2->lib_Revision >= MIN_OGLES2_LIB_REVISION)))  {
	        COBALT_LOGE("Warning, your OGLES2.library is too old, minimum is v%d.%d, please update!\n", MIN_OGLES2_LIB_VERSION,MIN_OGLES2_LIB_REVISION);
		}	
	  IOGLES2 = (struct OGLES2IFace *)so_exec.IExec->GetInterface(LOGLES2, "main", 1, NULL); 
	  if(!IOGLES2) {
	      COBALT_LOGE("Error, cannot open ogles2 Interface!\n");
  ...
  *lib = LOGLES2;
  ...
  COBALT_LOGD("OGLES2 Library and Interface open successfuly\n");
}
...
void os4CloseLib()
{
    ...
    COBALT_LOGD("OGLES2 Library and Interface closed\n");
}
...
void* os4GetProcAddress(const char* name)
{
    ...
    EX(glViewport)
    //EX(glPolygonMode) //This is a non-standard function, and cobalt will ignore it (and emulate it), even if Amiga OGLES2 driver implement it
