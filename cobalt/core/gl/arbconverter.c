...
char* cobalt_convertARB(const char* const code, int vertex, char **error_msg, int *error_ptr) {
	...
	do {
		if (vertex) {
			...
			if (specialCases.hasFogFragCoord) {
				APPEND_OUTPUT("\tvec4 cobalt_FogFragCoordTemp = vec4(gl_FogFragCoord);\n", 54)
			}
		} else {
			// No address
			APPEND_OUTPUT("#version 120\n\nvoid main() {\n", 28)
			if (specialCases.isDepthReplacing) {
				APPEND_OUTPUT("\tvec4 cobalt_FragDepthTemp = vec4(gl_FragDepth);\n", 48)
			}
		}
		...
		if (specialCases.hasFogFragCoord) {
			APPEND_OUTPUT("\tgl_FogFragCoord = cobalt_FogFragCoordTemp.x;\n", 45)
		}
		if (specialCases.isDepthReplacing) {
			APPEND_OUTPUT("\tgl_FragDepth = cobalt_FragDepthTemp.z;\n", 39)
		}
