...
int resolveOutput(sCurStatus_NewVar *newVar, int vertex, struct sSpecialCases *specialCases) {
	...
	if (!tok) {
		...
		if (!tok) {
			...
			pushArray((sArray*)&newVar->var->init, strdup("cobalt_FogFragCoordTemp"));
			newVar->var->init.strings_total_len = 22;
		...
		if (!tok) {
			...
			pushArray((sArray*)&newVar->var->init, strdup("cobalt_FragDepthTemp"));
			newVar->var->init.strings_total_len = 12;
		} else {
			ARBCONV_DBG_RE("Failed to get output: result.%s\n", tok)
			free(tok);
		...
		case VARTYPE_TEMP:
			...
			case TOK_IDENTIFIER: {
				...
				if (!strcmp(tok, "half")) {
					...
					pushArray((sArray*)curStatusPtr->curValue.newVar.var, strdup("cobalt_half"));
