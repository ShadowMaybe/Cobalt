...
GLvoid *copy_gl_array(const GLvoid *src,
    ...
    const char *unknown_str = "COBALT: copy_gl_array -> unknown type: %x\n";
    if(!dst) {
...
GLvoid *copy_gl_array_texcoord(const GLvoid *src,
    ...
    static const char *unknown_str = "COBALT: copy_gl_array -> unknown type: %x\n";
    if(!dst)
...
GLvoid *copy_gl_array_quickconvert(const GLvoid *src,
    ...
    const char *unknown_str = "COBALT: copy_gl_array_quickconvert -> unknown type: %x\n";
...
GLvoid *copy_gl_array_convert(const GLvoid *src,
    ...
    const char *unknown_str = "COBALT: copy_gl_array_convert -> unknown type: %x\n";
...
GLfloat *gl_pointer_index(vertexattrib_t *p, GLint index) {
    ...
    GL_TYPE_SWITCH(src, ptr, p->type,
        ...
        default:
            printf("COBALT: unknown pointer type: 0x%x\n", p->type);
    )
    return buf;
}
