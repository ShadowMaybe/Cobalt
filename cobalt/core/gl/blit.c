...
void cobalt_blitTexture_gles1(GLuint texture,
    ...
    if(drawtexok) {
        ...
        cobalt_glPushClientAttrib(GL_CLIENT_VERTEX_ARRAY_BIT | GL_CLIENT_PIXEL_STORE_BIT);
        cobalt_glGetFloatv(GL_TEXTURE_MATRIX, old_texture);
        cobalt_glGetFloatv(GL_PROJECTION_MATRIX, old_projection);
        cobalt_glGetFloatv(GL_MODELVIEW_MATRIX, old_modelview);
        cobalt_glMatrixMode(GL_TEXTURE);
        cobalt_glLoadIdentity();
        cobalt_glMatrixMode(GL_PROJECTION);
        cobalt_glLoadIdentity();
        cobalt_glMatrixMode(GL_MODELVIEW);
        cobalt_glLoadIdentity();
        ...
        cobalt_glMatrixMode(GL_TEXTURE);
        cobalt_glLoadMatrixf(old_texture);
        cobalt_glMatrixMode(GL_MODELVIEW);
        cobalt_glLoadMatrixf(old_modelview);
        cobalt_glMatrixMode(GL_PROJECTION);
        cobalt_glLoadMatrixf(old_projection);
    }
...
void cobalt_blitTexture_gles2(GLuint texture,
    GLfloat sx, GLfloat sy,
    GLfloat width, GLfloat height, 
    GLfloat nwidth, GLfloat nheight, 
    GLfloat zoomx, GLfloat zoomy, 
    ...
    if(!glstate->blit) {
        ...
        if (!success)
        {
            ...
            COBALT_LOGE("Failed to produce blit fragment shader.\n%s", log);
            free(glstate->blit);
            glstate->blit = NULL;
        }
        ...
        if (!success)
        {
            ...
            char log[400];
            host_functions.glGetShaderInfoLog(glstate->blit->pixelshader_alpha, 399, NULL, log);
            COBALT_LOGE("Failed to produce blit with alpha fragment shader.\n%s", log);
            free(glstate->blit);
            glstate->blit = NULL;
        }
        ...
        if( !success )
        {
            ...
            char log[400];
            host_functions.glGetShaderInfoLog(glstate->blit->pixelshader_alpha, 399, NULL, log);
            COBALT_LOGE("Failed to produce blit vertex shader.\n%s", log);
            free(glstate->blit);
            glstate->blit = NULL;
        }
        ...
        if( !success )
        {
            ...
            char log[400];
            host_functions.glGetShaderInfoLog(glstate->blit->pixelshader_alpha, 399, NULL, log);
            COBALT_LOGE("Failed to produce blit with alpha vertex shader.\n%s", log);
            free(glstate->blit);
            glstate->blit = NULL;
        }
        ...
        if( !success )
        {
            COBALT_LOGE("Failed to link blit program.\n");
            free(glstate->blit);
            glstate->blit = NULL;
        }
...
//printf("blitTexture(%d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d) customvp=%d, vp=%d/%d/%d/%d\n", texture, sx, sy, width, height, nwidth, nheight, zoomx, zoomy, vpwidth, vpheight, x, y, mode, (vpwidth>0.0), glstate->raster.viewport.x, glstate->raster.viewport.y, glstate->raster.viewport.width, glstate->raster.viewport.height);
    ...
    cobalt_glPushAttrib(GL_TEXTURE_BIT | GL_ENABLE_BIT | GL_TRANSFORM_BIT | GL_COLOR_BUFFER_BIT | GL_CURRENT_BIT);
    ...
    cobalt_glDisable(GL_DEPTH_TEST);
    cobalt_glDisable(GL_CULL_FACE);
    cobalt_glDisable(GL_STENCIL_TEST);

    if(depthwrite)
        cobalt_glDepthMask(GL_FALSE);
...
//printf("TMU%d, turning off Streaming (blit)\n", 0);
    ...
    if(hardext.esversion==1) {
        ...
        cobalt_blitTexture_gles1(texture, sx, sy, width, height, 
        ...
        cobalt_blitTexture_gles2(texture, sx, sy, width, height, 
            nwidth, nheight, zoomx, zoomy, 
            vpwidth, vpheight, x, y, mode);
    }
