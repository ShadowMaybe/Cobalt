...
void COBALT_API cobalt_glBufferData(GLenum target, GLsizeiptr size, const GLvoid * data, GLenum usage) {
    ...
    if(     (target==GL_ARRAY_BUFFER || target==GL_ELEMENT_ARRAY_BUFFER) 
         && (usage==GL_STREAM_DRAW || usage==GL_STATIC_DRAW || usage==GL_DYNAMIC_DRAW) && g_cobalt.usevbo)
        go_real = 1;
...
void COBALT_API cobalt_glBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, const GLvoid * data) {
    ...
    if (buff==NULL) {
		errorShim(GL_INVALID_OPERATION);
        DBG(printf("COBALT: Warning, null buffer for target=0x%04X for glBufferSubData\n", target);)
        return;
    }
...
void* APIENTRY_GLES cobalt_glMapNamedBuffer(GLuint buffer, GLenum access) {
...
void* COBALT_API cobalt_glMapBufferRange(GLenum target, GLintptr offset, GLsizeiptr length, GLbitfield access)
{
...
void bindBuffer(GLenum target, GLuint buffer)
{
    ...
    if(target==GL_ARRAY_BUFFER) {
        if(glstate->bind_buffer.array == buffer && !g_cobalt.buffer_force_bind)
            return;
        ...
        if(glstate->bind_buffer.index == buffer && !g_cobalt.buffer_force_bind)
            return;
