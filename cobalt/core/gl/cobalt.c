...
void COBALT_API cobalt_glVertexPointer(GLint size, GLenum type,
...
void COBALT_API cobalt_glColorPointer(GLint size, GLenum type,
                     GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API cobalt_glTexCoordPointer(GLint size, GLenum type,
                     GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API cobalt_glSecondaryColorPointer(GLint size, GLenum type, 
					GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API cobalt_glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid *pointer) {
    ...
    if (!stride)
        stride = tex * gl_sizeof(tf) +
                 ...
                 vert * gl_sizeof(vf);
    if (tex) {
		cobalt_glEnableClientState(GL_TEXTURE_COORD_ARRAY);
        cobalt_glTexCoordPointer(tex, tf, stride, (GLvoid *)ptr);
        ptr += tex * gl_sizeof(tf);
    }
    if (color) {
		cobalt_glEnableClientState(GL_COLOR_ARRAY);
        cobalt_glColorPointer(color, cf, stride, (GLvoid *)ptr);
        ptr += color * gl_sizeof(cf);
    }
    if (normal) {
		cobalt_glEnableClientState(GL_NORMAL_ARRAY);
        cobalt_glNormalPointer(nf, stride, (GLvoid *)ptr);
        ptr += normal * gl_sizeof(nf);
    }
    if (vert) {
		cobalt_glEnableClientState(GL_VERTEX_ARRAY);
        cobalt_glVertexPointer(vert, vf, stride, (GLvoid *)ptr);
    }
}
...
void COBALT_API cobalt_glEnd(void) {
    ...
    if(glstate->list.compiling) {
        ...
        if (!g_cobalt.beginend /*|| (glstate->polygon_mode==GL_LINE)*/) {
    ...
    if(withColor)
        cobalt_glColor4f(glstate->color[0], glstate->color[1], glstate->color[2], glstate->color[3]);
    noerrorShim();
}
    ...
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            ...
            if (!glstate->list.pending)
                return cobalt_glNormal3f(v[0], v[1], v[2]);  // this will put the call on the stack in the current list
        } else {
...
void COBALT_API cobalt_glColor4fv(GLfloat* v) {
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            if (glstate->list.compiling || glstate->list.active->stage<STAGE_DRAW) {
            ...
            if (!glstate->list.pending)
                return cobalt_glColor4f(v[0], v[1], v[2], v[3]);
        } else {
...
void COBALT_API cobalt_glMultiTexCoord2fv(GLenum target, GLfloat* v) {
	// TODO, error if target is unsuported texture....
    if (glstate->list.active) {
        if(glstate->list.pending)
            cobalt_flush();
...
void COBALT_API cobalt_glMultiTexCoord4fv(GLenum target, GLfloat* v) {
	// TODO, error if target is unsuported texture....
    if (glstate->list.active) {
        if(glstate->list.pending)
            cobalt_flush();
...
void COBALT_API cobalt_glArrayElement(GLint i) {
    ...
    if (p->enabled) {
        ...
        if(p->type==GL_FLOAT) {
            ...
            if(size==3)
                cobalt_glColor3ubv(b);
            else
                cobalt_glColor4ubv(b);
        } else {
    ...
    if (p->enabled) {
        ...
        if (size<4)
            cobalt_glTexCoord2fv(v);
        else
            cobalt_glTexCoord4fv(v);
    }
    for (int a=1; a<vao->maxtex; a++) {
	    ...
	    if (p->enabled) {
            ...
            if (p->size<4)
                cobalt_glMultiTexCoord2fv(GL_TEXTURE0+a, v);
            else
                cobalt_glMultiTexCoord4fv(GL_TEXTURE0+a, v);
	    }
    }
    ...
    if (p->enabled) {
        ...
        if (p->size == 4) {
            cobalt_glVertex4fv(v);
        } else if (p->size == 3) {
            cobalt_glVertex3fv(v);
        } else {
            cobalt_glVertex2fv(v);
        }
    }
}
...
void COBALT_API cobalt_glUnlockArrays(void) {
    if(g_cobalt.usevbo>1 && glstate->vao->locked==g_cobalt.usevbo) UnBuffer();
...
void ToBuffer(int first, int count) {
    ...
    glstate->vao->locked = g_cobalt.usevbo;
    ...
    cobalt_scratch_vertex(total);    // alloc if needed and bind scratch vertex buffer
...
static renderlist_t *cobalt_glGetList(GLuint list) {
    khint_t k;
...
void COBALT_API cobalt_glCallList(GLuint list) {
	noerrorShim();
    if (glstate->list.active) {
        glstate->list.active = append_calllist(glstate->list.active, cobalt_glGetList(list));
		return;
	}
    // TODO: the output of this call can be compiled into another display list
    renderlist_t *l = cobalt_glGetList(list);
    if (l)
        draw_renderlist(l);
}
...
void COBALT_API cobalt_glCallLists(GLsizei n, GLenum type, const GLvoid *lists) {
    ...
    #define call_bytes(name, stride)                             \
        case name:                                               \
            ...
            cobalt_glCallList(list + glstate->list.base);                  \
            break
...
void COBALT_API cobalt_glDeleteList(GLuint list) {
...
void amiga_pre_swap()
#else
NonAliasExportDecl(void,cobalt_pre_swap,())
#endif
{
    if (glstate->list.active) cobalt_flush();
    if (glstate->raster.bm_drawing) bitmap_flush();
...
#else
NonAliasExportDecl(void,cobalt_post_swap,())
#endif
{
		show_fps();
