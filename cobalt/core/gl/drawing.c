...
static renderlist_t *arrays_to_renderlist(renderlist_t *list, GLenum mode,
    ...
    if(glstate->vao->shared_arrays) {
        ...
        if(!g_cobalt.novaocache && glstate->vao != glstate->defaultvao) {
...
if(count>500000) return;
    ...
    if (mode == GL_QUADS) {
        ...
        int ilen = (count*3)/2;
        if (iindices) {
            cobalt_scratch(ilen*sizeof(GLuint));
            ...
            cobalt_scratch(ilen*sizeof(GLushort));
    ...
    if (glstate->render_mode == GL_SELECT) {
        ...
        if(hardext.esversion==1) {
            ...
            #define TEXTURE(A) cobalt_glClientActiveTexture(A+GL_TEXTURE0);
            ...
            for (int aa=0; aa<hardext.maxtex; aa++) {
                ...
                if (itarget>=0) {
                    if (!IS_TEX2D(glstate->enable.texture[aa]) && (IS_ANYTEX(glstate->enable.texture[aa]))) {
                        cobalt_glActiveTexture(GL_TEXTURE0+aa);
                        realize_active();
        ...
        if(hardext.esversion>1 && g_cobalt.usevbo==2 && glstate->vao->locked==1) {
        ...
        if(hardext.esversion>1 && g_cobalt.usevbo==3 && (glstate->vao->locked==1 || glstate->vao->locked==2)) {
...
#define MIN_BATCH  g_cobalt.minbatch
#define MAX_BATCH  g_cobalt.maxbatch
...
void COBALT_API cobalt_glDrawRangeElements(GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type, const void *indices) {
    ...
    if (compiling) {
        ...
        if(g_cobalt.mergelist && list->stage>=STAGE_DRAW && is_list_compatible(list) && !list->use_glstate && sindices) {
...
void COBALT_API cobalt_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
    ...
    if ((mode==GL_QUADS) && (count>4*8000)) {
        ...
        for (int i=0; i<count; i+=4*8000) {
            ...
            cobalt_glDrawArrays(mode, i, cnt);
        }
        return;
    }
	noerrorShim();
    ...
    if (glstate->list.active) {
        ...
        if(g_cobalt.mergelist && list->stage>=STAGE_DRAW && is_list_compatible(list) && !list->use_glstate) {
            list = NewDrawStage(list, mode);
            if(list->vert) {
...
void COBALT_API cobalt_glMultiDrawArrays(GLenum mode, const GLint *firsts, const GLsizei *counts, GLsizei primcount)
{
    ...
    for (int i=0; i<primcount; i++) {
        ...
        if (compiling) {
            if(g_cobalt.mergelist && glstate->list.active->stage>=STAGE_DRAW && is_list_compatible(glstate->list.active) && !glstate->list.active->use_glstate) {
...
void COBALT_API cobalt_glMultiDrawElements( GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount)
{
...
void COBALT_API cobalt_glMultiDrawElementsBaseVertex( GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount, const GLint * basevertex) {
...
void COBALT_API cobalt_glDrawElementsBaseVertex(GLenum mode, GLsizei count, GLenum type, const void *indices, GLint basevertex) {
    ...
    if(basevertex==0)
        cobalt_glDrawElements(mode, count, type, indices);
    else {
        count = adjust_vertices(mode, count);
...
void COBALT_API cobalt_glDrawRangeElementsBaseVertex(GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type, const void *indices, GLint basevertex) {
    ...
    if(basevertex==0) {
        cobalt_glDrawRangeElements(mode, start, end, count, type, indices);
    } else {
        count = adjust_vertices(mode, count);
...
void COBALT_API cobalt_glDrawElementsInstancedBaseVertex(GLenum mode, GLsizei count, GLenum type, const void *indices, GLsizei primcount, GLint basevertex) {
    ...
    if(basevertex==0)
        cobalt_glDrawElementsInstanced(mode, count, type, indices, primcount);
    else {
        count = adjust_vertices(mode, count);
