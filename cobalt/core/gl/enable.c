...
static void proxy_glEnable(GLenum cap, bool enable, void (APIENTRY_GLES *next)(GLenum)) {
    ...
    #define GO(constant, name) \
        case constant: if(glstate->list.pending && glstate->enable.name!=enable) cobalt_flush(); glstate->enable.name = enable; break;
    #define GONF(constant, name) \
        case constant: glstate->enable.name = enable; break;
    #define GOFPE(constant, name, fct) \
        case constant: if(glstate->list.pending && glstate->enable.name!=enable) cobalt_flush(); glstate->enable.name = enable; if(glstate->fpe_state) { fct; } break;
    ...
    // Alpha Hack
    if (g_cobalt.alphahack && (cap==GL_ALPHA_TEST) && enable) {
    ...
    switch (cap) {
        ...
        case GL_BLEND: if(glstate->enable.blend != enable) {FLUSH_BEGINEND; glstate->enable.blend = enable; if(glstate->fpe_state && g_cobalt.shaderblend) { glstate->fpe_state->blend_enable = enable; } else next(cap);} break;
        ...
        case GL_TEXTURE_1D:
            ...
            break;
        case GL_TEXTURE_2D:
            if(glstate->list.pending && ((glstate->enable.texture[glstate->texture.active]>>ENABLED_TEX2D)&1)!=enable) cobalt_flush();
