...
void COBALT_API fpe_ReleventState_DefaultVertex(fpe_state_t *dest, fpe_state_t *src, shaderconv_need_t* need)
{
...
void COBALT_API fpe_ReleventState(fpe_state_t *dest, fpe_state_t *src, int fixed)
{
    // filter out some non relevant state (like texture stuff if texture is disabled)
    memcpy(dest, src, sizeof(fpe_state_t));
    // alpha test
...
int COBALT_API fpe_IsEmpty(fpe_state_t *state) {
...
void COBALT_API fpe_oldprogram(fpe_state_t* state) {
    ...
    glstate->fpe->vert = cobalt_glCreateShader(GL_VERTEX_SHADER);
    if(state->vertex_prg_id) {
        cobalt_glShaderSource(glstate->fpe->vert, 1, fpe_CustomVertexShader(old_vtx->shader->source, state, state->fragment_prg_id?0:1), NULL);
        cobalt_glCompileShader(glstate->fpe->vert);
        cobalt_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            host_functions.glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
            if(g_cobalt.logshader)
                printf("COBALT: FPE ARB Vertex program compile failed: ARB source is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n", old_vtx->string, old_vtx->shader->source, buff);
            else
                printf("COBALT: FPE ARB Vertex program compile failed: %s\n", buff);
        }
        getShader(glstate->fpe->vert)->old = old_vtx;
    } else {
        // use fragment need to build default vertex shader
        cobalt_glShaderSource(glstate->fpe->vert, 1, fpe_VertexShader(&old_frg->shader->need, state), NULL);
        cobalt_glCompileShader(glstate->fpe->vert);
        cobalt_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            host_functions.glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
            printf("COBALT: FPE ARB Default Vertex program compile failed: %s\n", buff);
        }
    }
    cobalt_glAttachShader(glstate->fpe->prog, glstate->fpe->vert);
    glstate->fpe->frag = cobalt_glCreateShader(GL_FRAGMENT_SHADER);
    if(state->fragment_prg_id) {
        cobalt_glShaderSource(glstate->fpe->frag, 1, fpe_CustomFragmentShader(old_frg->shader->source, state), NULL);
        cobalt_glCompileShader(glstate->fpe->frag);
        cobalt_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            host_functions.glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
            if(g_cobalt.logshader)
                printf("COBALT: FPE ARB Fragment program compile failed: ARB source is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n", old_frg->string, old_frg->shader->source, buff);
            else
                printf("COBALT: FPE ARB Fragment program compile failed: %s\n", buff);
        }
        getShader(glstate->fpe->frag)->old = old_frg;
    } else {
        // use vertex need to build default fragment shader
        cobalt_glShaderSource(glstate->fpe->frag, 1, fpe_FragmentShader(&old_vtx->shader->need, state), NULL);
        cobalt_glCompileShader(glstate->fpe->frag);
        cobalt_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            host_functions.glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
            printf("COBALT: FPE ARB Default Fragment program compile failed: %s\n", buff);
        }
    }
    cobalt_glAttachShader(glstate->fpe->prog, glstate->fpe->frag);
    // Ok, and now link the program
    cobalt_glLinkProgram(glstate->fpe->prog);
    cobalt_glGetProgramiv(glstate->fpe->prog, GL_LINK_STATUS, &status);
    if(status!=GL_TRUE) {
        char buff[1000];
        host_functions.glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL, buff);
        if(g_cobalt.logshader)
            printf("COBALT: FPE ARB Program link failed: %s\n with vertex %s%s%s%s%s and fragment %s%s%s%s%s\n", 
                buff, 
...
void COBALT_API fpe_program(int ispoint) {
    ...
    if(glstate->fpe->glprogram==NULL) {
        glstate->fpe->prog = cobalt_glCreateProgram();
        DBG(int from_psa = 1;)
        if(fpe_GetProgramPSA(glstate->fpe->prog, &state)==0) {
            ...
            if(state.vertex_prg_id || state.fragment_prg_id) {
                ...
                glstate->fpe->vert = cobalt_glCreateShader(GL_VERTEX_SHADER);
                cobalt_glShaderSource(glstate->fpe->vert, 1, fpe_VertexShader(NULL, glstate->fpe_state), NULL);
                cobalt_glCompileShader(glstate->fpe->vert);
                cobalt_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
                if(status!=GL_TRUE) {
                    char buff[1000];
                    host_functions.glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
                    if(g_cobalt.logshader)
                        printf("COBALT: FPE Vertex shader compile failed: source is\n%s\n\nError is: %s\n", fpe_VertexShader(NULL, glstate->fpe_state)[0], buff);
                    else
                        printf("COBALT: FPE Vertex shader compile failed: %s\n", buff);
                }
                glstate->fpe->frag = cobalt_glCreateShader(GL_FRAGMENT_SHADER);
                cobalt_glShaderSource(glstate->fpe->frag, 1, fpe_FragmentShader(NULL, glstate->fpe_state), NULL);
                cobalt_glCompileShader(glstate->fpe->frag);
                cobalt_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
                if(status!=GL_TRUE) {
                    char buff[1000];
                    host_functions.glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
                    if(g_cobalt.logshader)
                        printf("COBALT: FPE Fragment shader compile failed: source is\n%s\n\nError is: %s\n", fpe_FragmentShader(NULL, glstate->fpe_state)[0], buff);
                    else
                        printf("COBALT: FPE Fragment shader compile failed: %s\n", buff);
                }
                ...
                if(status!=GL_TRUE) {
                    ...
                    host_functions.glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL, buff);
                    if(g_cobalt.logshader) {
                        printf("COBALT: FPE Program link failed: source of vertex shader is\n%s\n\n", fpe_VertexShader(NULL, glstate->fpe_state)[0]);
                        printf("source of fragment shader is \n%s\n\nError is: %s\n", fpe_FragmentShader(NULL, glstate->fpe_state)[0], buff);
                    } else
                        printf("COBALT: FPE Program link failed: %s\n", buff);
                }
...
program_t* COBALT_API fpe_CustomShader(program_t* glprogram, fpe_state_t* state)
{
    ...
    if(fpe->glprogram==NULL) {
        GLint status;
        fpe->vert = cobalt_glCreateShader(GL_VERTEX_SHADER);
        cobalt_glShaderSource(fpe->vert, 1, fpe_CustomVertexShader(glprogram->last_vert->source, state, 0), NULL);
        cobalt_glCompileShader(fpe->vert);
        cobalt_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            cobalt_glGetShaderInfoLog(fpe->vert, 1000, NULL, buff);
            printf("COBALT: FPE Custom Vertex shader compile failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        fpe->frag = cobalt_glCreateShader(GL_FRAGMENT_SHADER);
        cobalt_glShaderSource(fpe->frag, 1, fpe_CustomFragmentShader(glprogram->last_frag->source, state), NULL);
        cobalt_glCompileShader(fpe->frag);
        cobalt_glGetShaderiv(fpe->frag, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            cobalt_glGetShaderInfoLog(fpe->frag, 1000, NULL, buff);
            printf("COBALT: FPE Custom Fragment shader compile failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        fpe->prog = cobalt_glCreateProgram();
        cobalt_glAttachShader(fpe->prog, fpe->vert);
        cobalt_glAttachShader(fpe->prog, fpe->frag);
        ...
        cobalt_glLinkProgram(fpe->prog);
        cobalt_glGetProgramiv(fpe->prog, GL_LINK_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            cobalt_glGetProgramInfoLog(fpe->prog, 1000, NULL, buff);
            printf("COBALT: FPE Custom Program link failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        // now find the program
        khint_t k_program;
...
program_t* COBALT_API fpe_CustomShader_DefaultVertex(program_t* glprogram, fpe_state_t* state_vertex)
{
    // state is not empty and glprogram already has some cache (it may be empty, but kh'thingy is initialized)
    // TODO: what if program is composed of more then 1 vertex or fragment shader?
    ...
    if(fpe->glprogram==NULL) {
        GLint status;
        fpe->vert = cobalt_glCreateShader(GL_VERTEX_SHADER);
        cobalt_glShaderSource(fpe->vert, 1, fpe_VertexShader(glprogram->default_need, state_vertex), NULL);
        cobalt_glCompileShader(fpe->vert);
        cobalt_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
            cobalt_glGetShaderInfoLog(fpe->vert, 1000, NULL, buff);
            printf("COBALT: FPE Default Vertex shader compile failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        fpe->frag = cobalt_glCreateShader(GL_FRAGMENT_SHADER);
        cobalt_glShaderSource(fpe->frag, 1, fpe_CustomFragmentShader(glprogram->last_frag->source, state_vertex), NULL);
        cobalt_glCompileShader(fpe->frag);
        cobalt_glGetShaderiv(fpe->frag, GL_COMPILE_STATUS, &status);
        if(status!=GL_TRUE) {
            char buff[1000];
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            cobalt_glGetProgramInfoLog(fpe->prog, 1000, NULL, buff);
            printf("COBALT: FPE Custom Program with Default Vertex link failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        // now find the program
        khint_t k_program;
...
void COBALT_API fpe_SyncUniforms(uniformcache_t *cache, program_t* glprogram) {
    //TODO: Optimize this...
        ...
        if(m->parent_size) {
            ...
            switch(m->type) {
                ...
                case GL_FLOAT_MAT4:
                    ...
                    break;
                default:
                    printf("COBALT: Warning, sync uniform on father/son program with unknown uniform type %s\n", PrintEnum(m->type));
            }
        }
    );
...
void COBALT_API fpe_glClientActiveTexture(GLenum texture) {
    DBG(printf("fpe_glClientActiveTexture(%s)\n", PrintEnum(texture));)
}

void COBALT_API fpe_EnableDisableClientState(GLenum cap, GLboolean val) {
    int att = -1;
        switch(cap) {
...
void COBALT_API fpe_glEnableClientState(GLenum cap) {
    DBG(printf("fpe_glEnableClientState(%s)\n", PrintEnum(cap));)
    fpe_EnableDisableClientState(cap, GL_TRUE);
}

void COBALT_API fpe_glDisableClientState(GLenum cap) {
    DBG(printf("fpe_glDisableClientState(%s)\n", PrintEnum(cap));)
    fpe_EnableDisableClientState(cap, GL_FALSE);
}

void COBALT_API fpe_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) {
}

void COBALT_API fpe_glSecondaryColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API fpe_glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API fpe_glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API fpe_glNormalPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API fpe_glTexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
    fpe_glTexCoordPointerTMU(size, type, stride, pointer, glstate->texture.client);
}

void COBALT_API fpe_glTexCoordPointerTMU(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer, int TMU) {
...
void COBALT_API fpe_glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
...
void COBALT_API fpe_glEnable(GLenum cap) {
    cobalt_glEnable(cap);    // may reset fpe curent program
}
void COBALT_API fpe_glDisable(GLenum cap) {
    cobalt_glDisable(cap);   // may reset fpe curent program
}

void COBALT_API fpe_glColor4f(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) {
    noerrorShim();
}

void COBALT_API fpe_glNormal3f(GLfloat nx, GLfloat ny, GLfloat nz) {
    noerrorShim();
}

void COBALT_API fpe_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
...
void COBALT_API fpe_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices) {
...
void COBALT_API fpe_glDrawArraysInstanced(GLenum mode, GLint first, GLsizei count, GLsizei primcount) {
...
void COBALT_API fpe_glDrawElementsInstanced(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices, GLsizei primcount) {
...
void COBALT_API fpe_glMatrixMode(GLenum mode) {
    noerrorShim();
}

void COBALT_API fpe_glLightModelf(GLenum pname, GLfloat param) {
    noerrorShim();
}
void COBALT_API fpe_glLightModelfv(GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void COBALT_API fpe_glLightfv(GLenum light, GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void COBALT_API fpe_glMaterialfv(GLenum face, GLenum pname, const GLfloat *params) {
    noerrorShim();
}
void COBALT_API fpe_glMaterialf(GLenum face, GLenum pname, const GLfloat param) {
...
void COBALT_API fpe_glFogfv(GLenum pname, const GLfloat* params) {
    noerrorShim();
...
void COBALT_API fpe_glPointParameterfv(GLenum pname, const GLfloat * params) {
    noerrorShim();
}
void COBALT_API fpe_glPointSize(GLfloat size) {
    noerrorShim();
}

void COBALT_API fpe_glAlphaFunc(GLenum func, GLclampf ref) {
    noerrorShim();
    int f = FPE_ALWAYS;
    switch(func) {
...
void realize_glenv(int ispoint, int first, int count, GLenum type, const void* indices, scratch_t* scratch) {
    ...
    if(g_cobalt.fbounbind && glstate->fbo.current_fb->id) {
        ...
        if(need && tex) {
            DBG(printf("COBALT: Need to Bind/Unbind FBO!");)
            ...
            if(dirty || v->size!=w->size || v->type!=w->type || v->normalized!=w->normalized 
                ...
                if((w->size==GL_BGRA || w->type==GL_DOUBLE) && scratch->size<8) { 
                    ...
                    if(w->size==GL_BGRA) {
                        ...
                        static int warn = 1;
                        if(warn) {
                            printf("COBALT: VertexAttribArray using GL_DOUBLE unimplemented!\n");
                            warn=0;
                        }
                    }
                } else {
                ...
                // chromebooks and some samsung phone. Partially due to how the cobalt counterpart method had been implemented
                /*
                if (v->integer) {
...
const char* cobalt_code = "_cobalt_";
const char* lightsource_code = "_cobalt_LightSource[";
const char* lightsource_fpe_code = "_cobalt_LightSource_";
const char* lightmodel_code = "_cobalt_LightModel.";
const char* frontmaterial_code = "_cobalt_FrontMaterial";
const char* backmaterial_code = "_cobalt_BackMaterial";
const char* frontmaterial_fpe_code = "_cobalt_FrontMaterial_shininess";
const char* backmaterial_fpe_code = "_cobalt_BackMaterial_shininess";
const char* frontlightmodelprod_code = "_cobalt_FrontLightModelProduct";
const char* backlightmodelprod_code = "_cobalt_BackLightModelProduct";
const char* frontlightprod_code = "_cobalt_FrontLightProduct[";
const char* backlightprod_code = "_cobalt_BackLightProduct[";
const char* frontlightprod_fpe_code = "_cobalt_FrontLightProduct_";
const char* backlightprod_fpe_code = "_cobalt_BackLightProduct_";
const char* normalrescale_code = "_cobalt_NormalScale";
const char* instanceID_code = "_cobalt_InstanceID";
const char* clipplanes_code = "_cobalt_ClipPlane[";
const char* clipplanes_fpe_code = "_cobalt_ClipPlane_";
const char* point_code = "_cobalt_Point";
const char* texenvcolor_code = "_cobalt_TextureEnvColor[";
const char* texenvcolor_fpe_code = "_cobalt_TextureEnvColor_";
const char* texenvcolor_noa_code = "_cobalt_TextureEnvColor";
const char* texgeneyestart_code = "_cobalt_EyePlane";
const char* texgeneye_code = "_cobalt_EyePlane%c[";
const char* texgeneye_fpe_code = "_cobalt_EyePlane%c_";
const char* texgeneye_noa_code = "_cobalt_EyePlane%c";
const char* texgenobjstart_code = "_cobalt_ObjectPlane";
const char* texgenobj_code = "_cobalt_ObjectPlane%c[";
const char* texgenobj_fpe_code = "_cobalt_ObjectPlane%c_";
const char* texgenobj_noa_code = "_cobalt_ObjectPlane%c";
const char texgenCoords[4] = {'S', 'T', 'R', 'Q'};
const char* alpharef_code = "_cobalt_AlphaRef";
const char* fpetexSampler_code = "_cobalt_TexSampler_";
const char* fpetexenvRGBScale_code = "_cobalt_TexEnvRGBScale_";
const char* fpetexenvAlphaScale_code = "_cobalt_TexEnvAlphaScale_";
const char* fpetexAdjust_code = "_cobalt_TexAdjust_";
const char* fog_code = "_cobalt_Fog.";
const char* vtx_progenv_noa = "_cobalt_Vertex_ProgramEnv_";
const char* vtx_progenv_arr = "_cobalt_Vertex_ProgramEnv[";
const char* vtx_progloc_noa = "_cobalt_Vertex_ProgramLocal_";
const char* vtx_progloc_arr = "_cobalt_Vertex_ProgramLocal[";
const char* frg_progenv_noa = "_cobalt_Fragment_ProgramEnv_";
const char* frg_progenv_arr = "_cobalt_Fragment_ProgramEnv[";
const char* frg_progloc_noa = "_cobalt_Fragment_ProgramLocal_";
const char* frg_progloc_arr = "_cobalt_Fragment_ProgramLocal[";
const char* samplers1d_noa = "_cobalt_Sampler1D_";
const char* samplers2d_noa = "_cobalt_Sampler2D_";
const char* samplers3d_noa = "_cobalt_Sampler3D_";
const char* samplersCube_noa = "_cobalt_SamplerCube_";
const char* blend_color_code = "_cobalt_BlendColor";
int builtin_CheckUniform(program_t *glprogram, char* name, GLint id, int size) {
    if(strncmp(name, cobalt_code, strlen(cobalt_code)))
        return 0;   // doesn't start with "_cobalt_", no need to look further
