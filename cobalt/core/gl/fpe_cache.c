...
static const char PSA_SIGN[] = "cobalt PrecompiledShaderArchive";
...
void fpe_disposeCache(fpe_cache_t* cache, int freeprog) {
    ...
    kh_foreach_value(cache, m, 
        if(freeprog) {
            if(m->glprogram)
                cobalt_glDeleteProgram(m->glprogram->id);
        }
        free(m);
    )
...
typedef struct cobalt_psa_s {
    int             size;
    int             dirty;
    kh_psalist_t*   cache;    
} cobalt_psa_t;

static cobalt_psa_t *psa = NULL;
...
void fpe_readPSA()
{
    ...
    fclose(f);
    COBALT_LOGD("Loaded a PSA with %d Precompiled Programs\n", psa->size);
}
...
void fpe_writePSA()
{
    ...
    fclose(f);
    COBALT_LOGD("Saved a PSA with %d Precompiled Programs\n", psa->size);
}
...
void fpe_InitPSA(const char* name)
{
    ...
    psa = (cobalt_psa_t*)calloc(1, sizeof(cobalt_psa_t));
...
int fpe_GetProgramPSA(GLuint program, fpe_state_t* state)
{
    ...
    // try to load...
    return cobalt_useProgramBinary(program, p->size, p->format, p->prog);
}
...
void fpe_AddProgramPSA(GLuint program, fpe_state_t* state)
{
    ...
    int l = cobalt_getProgramBinary(program, &p->size, &p->format, &p->prog);
