...
GLenum COBALT_API cobalt_glGetError(void) {
    DBG(printf("glGetError(), noerror=%d, type_error=%d shim_error=%s\n", g_cobalt.noerror, glstate->type_error, PrintEnum(glstate->shim_error));)
    if(g_cobalt.noerror)
        return GL_NO_ERROR;
        ...
        case GL_RENDERER:
            {
                ...
                return (GLubyte *)host_functions.glGetString(name);
            }
	case GL_SHADING_LANGUAGE_VERSION:
            if(g_cobalt.gl==21)
            return (GLubyte *)"1.20 via cobalt";
            else if(g_cobalt.gl==20)
                return (GLubyte *)"1.10 via cobalt";
			return (GLubyte *)"";
