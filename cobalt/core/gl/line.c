...
void COBALT_API cobalt_glLineStipple(GLuint factor, GLushort pattern) {
    ...
    if(pattern!=glstate->linestipple.pattern || factor!=glstate->linestipple.factor || !glstate->linestipple.texture) {
        ...
        if (! glstate->linestipple.texture) {
            cobalt_glGenTextures(1, &glstate->linestipple.texture);
            cobalt_glBindTexture(GL_TEXTURE_2D, glstate->linestipple.texture);
            cobalt_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
            cobalt_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
            cobalt_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
