...
//printf("draw_renderlist %p, size=%i, mode=%s(%s), ilen=%d, next=%p, color=%p, secondarycolor=%p fogcoord=%p\n", list, list->len, PrintEnum(list->mode), PrintEnum(list->mode_init), list->ilen, list->next, list->color, list->secondary, list->fogcoord);
    ...
    cobalt_glPushClientAttrib(GL_CLIENT_VERTEX_ARRAY_BIT);
    ...
    do {
        ...
        if (list->pushattribute)
            cobalt_glPushAttrib(list->pushattribute);
        ...
        if(list->render_op) {
            switch(list->render_op) {
                case 1: cobalt_glInitNames(); break;
                case 2: cobalt_glPopName(); break;
                case 3: cobalt_glPushName(list->render_arg); break;
                case 4: cobalt_glLoadName(list->render_arg); break;
            }
        }
        if (list->fog_op) {
            cobalt_glFogfv(GL_FOG_COLOR, list->fog_val);
        }
        if (list->pointparam_op) {
            switch (list->pointparam_op) {
                case 1: // GL_POINT_DISTANCE_ATTENUATION 
                    cobalt_glPointParameterfv(GL_POINT_DISTANCE_ATTENUATION , list->pointparam_val);
                    break;
            }
        }
        if (list->matrix_op) {
            switch (list->matrix_op) {
                case 1: // load
                    cobalt_glLoadMatrixf(list->matrix_val);
                    break;
                case 2: // mult
                    cobalt_glMultMatrixf(list->matrix_val);
                    break;
            }
        }
        if (list->set_tmu) {
            cobalt_glActiveTexture(GL_TEXTURE0+list->tmu);
        }
	    if (list->set_texture) {
            cobalt_glBindTexture(list->target_texture, list->texture);
        }
        // raster
        if (list->raster_op) {
            if (list->raster_op==1) {
                cobalt_glRasterPos3f(list->raster_xyz[0], list->raster_xyz[1], list->raster_xyz[2]);
            } else if (list->raster_op==2) {
                cobalt_glWindowPos3f(list->raster_xyz[0], list->raster_xyz[1], list->raster_xyz[2]);
            } else if (list->raster_op==3) {
                cobalt_glPixelZoom(list->raster_xyz[0], list->raster_xyz[1]);
            } else if ((list->raster_op&0x10000) == 0x10000) {
                cobalt_glPixelTransferf(list->raster_op&0xFFFF, list->raster_xyz[0]);
            }
        }
        if (list->raster) {
        ...
        if (list->bitmaps) {
            for (int i=0; i<list->bitmaps->count; i++) {
                ...
                cobalt_glBitmap(l->width, l->height, l->xorig, l->yorig, l->xmove, l->ymove, l->bitmap);
            }
        }
        ...
        if (list->material) {
            ...
            kh_foreach_value(map, m,
                switch (m->pname) {
                    case GL_SHININESS:
                        cobalt_glMaterialf(m->face,  m->pname, m->color[0]);
                        break;
                    default:
                        cobalt_glMaterialfv(m->face, m->pname, m->color);
                }
            )
        }
        if (list->colormat_face)
            cobalt_glColorMaterial(list->colormat_face, list->colormat_mode);
        if (list->light) {
            ...
            kh_foreach_value(lig, m,
                switch (m->pname) {
                    default:
                        cobalt_glLightfv(m->which, m->pname, m->color);
                }
            )
        }
        if (list->lightmodel) {
            cobalt_glLightModelfv(list->lightmodelparam, list->lightmodel);
        }

        if (list->linestipple_op) {
            cobalt_glLineStipple(list->linestipple_factor, list->linestipple_pattern);
        }
        ...
        if (list->texenv) {
            ...
            kh_foreach_value(tgn, m,
                cobalt_glTexEnvfv(m->target, m->pname, m->params);
            )
        }
        ...
        if (list->texgen) {
            ...
            rendertexgen_t *m;
            kh_foreach_value(tgn, m,
                cobalt_glTexGenfv(m->coord, m->pname, m->color);
            )
        }
        
        if (list->polygon_mode) {
            cobalt_glPolygonMode(GL_FRONT_AND_BACK, list->polygon_mode);
        }
        ...
        if(!use_vbo_array && (hardext.esversion==1 || g_cobalt.usevbo==0 || !list->name)) {
            use_vbo_array = 1;
        }
        int use_vbo_indices = list->use_vbo_indices;
        if(!use_vbo_indices &&  (hardext.esversion==1 || g_cobalt.usevbo==0 || !list->name)) {
            use_vbo_indices = 1;
        }
        if (list->vert) {
...
//printf("colors=%f, %f, %f, %f / %f, %f, %f, %f\n", list->color[0],list->color[1],list->color[2],list->color[3], list->color[4],list->color[5],list->color[6],list->color[7]);
        ...
        #define TEXTURE(A) if (cur_tex!=A) {cobalt_glClientActiveTexture(A+GL_TEXTURE0); cur_tex=A;}
        stipple = false;
        ...
        if (stipple) {
            ...
            if(glstate->gleshard->active!=stipple_tmu) {
                ...
                cobalt_glActiveTexture(GL_TEXTURE0+stipple_tmu);
            }
            TEXTURE(stipple_tmu);
            GLenum matmode;
            cobalt_glGetIntegerv(GL_MATRIX_MODE, (GLint *) &matmode);
            cobalt_glMatrixMode(GL_TEXTURE);
            cobalt_glPushMatrix();
            cobalt_glLoadIdentity();
            cobalt_glMatrixMode(matmode);
            stipple_env = glstate->texenv[stipple_tmu].env.mode;
            cobalt_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
            stipple_tex2d = cobalt_glIsEnabled(GL_TEXTURE_2D);
            stipple_alpha = cobalt_glIsEnabled(GL_ALPHA_TEST);
            cobalt_glEnable(GL_TEXTURE_2D);
            cobalt_glEnable(GL_ALPHA_TEST);
            for (int k=0; k<4; k++) {
                stipple_texgen[k] = cobalt_glIsEnabled(GL_TEXTURE_GEN_S+k);
                if(stipple_texgen[k])
                    cobalt_glDisable(GL_TEXTURE_GEN_S+k);
            }
            stipple_afunc = glstate->alphafunc;
            stipple_aref = glstate->alpharef;
            cobalt_glAlphaFunc(GL_GREATER, 0.0f);
            bind_stipple_tex();
        ...
        if(hardext.esversion==1) {
            for (int a=0; a<hardext.maxtex; a++) {
                if(glstate->enable.texture[a] || (stipple && a==stipple_tmu)) {
                    ...
                    if((list->tex[a] || (use_texgen[a] && !needclean[a])) && ((!(g_cobalt.texmat || glstate->texture_matrix[a]->identity)) || (bound->adjust))) {
                        if(!use_texgen[a]) {
                            RS(a, list->len);
				...
				int stride = list->tex_stride[a]>>2;    // stride need to be a multiple of 4 (i.e. sizeof(GLfloat))
                        ...
                        if (!(g_cobalt.texmat || glstate->texture_matrix[a]->identity))
...
//else if (!glstate->enable.texgen_s[a] && glstate->enable.texture[a]) printf("COBALT: texture[%i] without TexCoord, mode=0x%04X (init=0x%04X), listlen=%i\n", a, list->mode, list->mode_init, list->len);
        ...
        if (stipple) {
            ...
            cobalt_glPopMatrix();
            cobalt_glMatrixMode(matmode);
            cobalt_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, stipple_env);
            cobalt_glAlphaFunc(stipple_afunc, stipple_aref);
            if(stipple_tex2d)
                cobalt_glEnable(GL_TEXTURE_2D);
            else
                cobalt_glDisable(GL_TEXTURE_2D);
            if(stipple_alpha)
                cobalt_glEnable(GL_ALPHA_TEST);
            else
                cobalt_glDisable(GL_ALPHA_TEST);
            for (int k=0; k<4; k++) {
                if(stipple_texgen[k])
                    cobalt_glEnable(GL_TEXTURE_GEN_S+k);
            }
            if(glstate->gleshard->active!=stipple_old)
                cobalt_glActiveTexture(GL_TEXTURE0+stipple_old);
        }
        if(list->post_color) cobalt_glColor4fv(list->post_colors);
        if(list->post_normal) cobalt_glNormal3fv(list->post_normals);
    } while ((list = list->next));
    cobalt_glPopClientAttrib();
}
