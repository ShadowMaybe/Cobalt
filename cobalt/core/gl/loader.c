...
void (COBALT_API *cobalt_getMainFBSize)(GLint* width, GLint* height);
...
void *open_lib(const char **names, const char *override) {
    ...
    if(!sanitizer && g_cobalt.deepbind)
        flags |= RTLD_DEEPBIND;
#endif
    if (override) {
        if ((lib = dlopen(override, flags))) {
            strncpy(path_name, override, PATH_MAX);
            if(!g_cobalt.nobanner) LOGD("COBALT:loaded: %s\n", path_name);
            return lib;
        } else {
            LOGE("COBALT_GLES override failed: %s\n", dlerror());
        }
    }
        ...
        for (int i = 0; names[i]; i++) {
            for (int e = 0; lib_ext[e]; e++) {
                ...
                if ((lib = dlopen(path_name, flags))) {
                    if(!g_cobalt.nobanner) LOGD("loaded: %s\n", path_name);
                    return lib;
                }
            }
        }
...
void load_libs() {
    ...
    first = 0;
#ifndef _WIN32
    const char *gles_override = GetEnvVar("COBALT_GLES");
...
#endif
    gles = open_lib((g_cobalt.es==1)?gles_lib:gles2_lib, gles_override);
#else
    gles = open_lib(L"COBALT_GLES", L"libGLESv2.dll");
#endif
    WARN_NULL(gles);
    ...
    egl = gles;
#elif !defined(_WIN32)
    const char *egl_override = GetEnvVar("COBALT_EGL");
    egl = open_lib(egl_lib, egl_override);
#else
    egl = open_lib(L"COBALT_EGL", L"libEGL.dll");
#endif
    WARN_NULL(egl);

#ifndef NO_GBM
    const char *gbm_override = GetEnvVar("COBALT_GBM");
    gbm = open_lib(gbm_lib, gbm_override);
    const char *drm_override = GetEnvVar("COBALT_DRM");
...
void* (COBALT_API *gles_getProcAddress)(const char *name);

void* COBALT_API proc_address(void *lib, const char *name) {
