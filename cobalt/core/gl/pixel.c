...
static const colorlayout_t *get_color_map(GLenum format) {
    ...
    switch (format) {
        ...
        default:
            printf("COBALT: unknown pixel format %s\n", PrintEnum(format));
            break;
    }
...
bool remap_pixel(const GLvoid *src, GLvoid *dst,
    ...
    switch (src_type) {
        ...
        type_case(GL_UNSIGNED_INT, GLubyte, read_each(, / (float)0xffffffff,))
        ...
        default:
            // TODO: add glSetError?
            printf("COBALT: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (dst_type) {
        ...
        default:
            printf("COBALT: Unsupported target data type: %s\n", PrintEnum(dst_type));
            return false;
            break;
    }
    return true;
...
bool transform_pixel(const GLvoid *src, GLvoid *dst,
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            printf("COBALT: transform_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
...
bool half_pixel(const GLvoid *src0, const GLvoid *src1,
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            printf("COBALT: half_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (src_type) {
       ...
       // TODO: force 565 to RGB? then we can change [4] -> 3
        ...
        default:
            printf("COBALT: half_pixel: Unsupported target data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    return true;
...
bool quarter_pixel(const GLvoid *src[16],
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            printf("COBALT: quarter_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (src_type) {
       ...
       // TODO: force 565 to RGB? then we can change [4] -> 3
        ...
        default:
            printf("COBALT: quarter_pixel Unsupported target data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    return true;
...
/*    if (new_width*2!=width || new_height*2!=height) {
        printf("COBALT: halfscaling %ux%u failed\n", width, height);
        return false;
    }*/
    //printf("COBALT: halfscaling %ux%u -> %ux%u (%s / %s)\n", width, height, new_width, new_height, PrintEnum(format), PrintEnum(type));
    const colorlayout_t *src_color;
    ...
    if(!src_color->type) {
        if(!pixel_size) {
            printf("COBALT: Cannot halfscale unknown format/type %s/%s\n", PrintEnum(format), PrintEnum(type));
            free(dst);
            return 0;
        }
        for (int y = 0; y < new_height; y++) {
...
bool pixel_thirdscale(const GLvoid *old, GLvoid **new,
    ...
    if (new_width*2!=width || new_height*2!=height || format!=GL_RGBA || type!=GL_UNSIGNED_BYTE) {
        //printf("COBALT: thirdscaling %ux%u failed\n", width, height);
        return false;
    }
//    printf("COBALT: halfscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
    uintptr_t src, pos, pix0, pix1, pix2, pix3;
...
/*    if (new_width*4!=width || new_height*4!=height) {
        printf("COBALT: quarterscaling %ux%u failed\n", width, height);
        return false;
    }*/
//    printf("COBALT: quarterscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
    ...
    if(!src_color->type) {
        if(!pixel_size) {
            printf("COBALT: Cannot quarterscale unknown format/type %s/%s\n", PrintEnum(format), PrintEnum(type));
            free(dst);
            return 0;
        }
        for (int y = 0; y < new_height; y++) {
...
bool pixel_doublescale(const GLvoid *old, GLvoid **new,
    ...
    new_width = width * 2;
    new_height = height * 2;
    //printf("COBALT: doublescaling %ux%u -> %ux%u (%s / %s)\n", width, height, new_width, new_height, PrintEnum(format), PrintEnum(type));
    const colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
