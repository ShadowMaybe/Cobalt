...
static void fill_program(program_t *glprogram)
{
    ...
    for (int i=0; i<n; i++) {
            ...
            if(id!=-1) {
                for (int j = 0; j<size; j++) {
                    ...
                    if(j) {
                        ...
                        // Renew id with index
                        id = host_functions.glGetUniformLocation(glprogram->id, gluniform->name);
                    } else
        ...
        DBG(else printf("COBALT: Warning, getting Uniform #%d info failed with %s\n", i, PrintEnum(e2));)
    }
    free(name);
    // reset uniform cache
    ...
    for (int i=0; i<n; i++) {
        ...
        DBG(else printf("COBALT: Warning, getting Attrib #%d info failed with %s\n", i, PrintEnum(e2));)
    }
    free(name);
}

int cobalt_useProgramBinary(GLuint program, int length, GLenum format, const void* binary)
{
...
int cobalt_getProgramBinary(GLuint program, int *length, GLenum *format, void** binary)
{
    if(hardext.prgbin_n==0)
        return 0;
...
void COBALT_API cobalt_glGetProgramBinary(GLuint program, GLsizei bufSize, GLsizei *length, GLenum *binaryFormat, void *binary)
{
    if(hardext.prgbin_n==0) {
        errorShim(GL_INVALID_OPERATION);
        return;
...
void COBALT_API cobalt_glProgramBinary(GLuint program, GLenum binaryFormat, const void *binary, GLsizei length)
{
    if(hardext.prgbin_n==0) {
        errorShim(GL_INVALID_OPERATION);
        return;
    ...
    if(cobalt_useProgramBinary(program, length, binaryFormat, binary))
        noerrorShim();
    else
        errorShim(GL_INVALID_OPERATION);
}
...
void COBALT_API cobalt_glLinkProgram(GLuint program) {
    ...
    if(!glprogram->last_vert) {
        ...
        GLenum vtx = cobalt_glCreateShader(GL_VERTEX_SHADER);
        cobalt_glShaderSource(vtx, 1, fpe_VertexShader(&needs, NULL), NULL);
        cobalt_glCompileShader(vtx);
        cobalt_glAttachShader(glprogram->id, vtx);
    }
    ...
    if(!glprogram->last_frag) {
        ...
        GLenum vtx = cobalt_glCreateShader(GL_FRAGMENT_SHADER);
        cobalt_glShaderSource(vtx, 1, fpe_FragmentShader(&needs, NULL), NULL);
        cobalt_glCompileShader(vtx);
        cobalt_glAttachShader(glprogram->id, vtx);
    }
    int compatible = 1;
    ...
    if(glprogram->last_vert)
        for (int i=0; i<ATT_MAX; ++i) {
            ...
            if(attribute)
                cobalt_glBindAttribLocation(glprogram->id, i, attribute);
        }
    ...
    if(host_functions.glLinkProgram) {
        ...
        if(glprogram->linked) {
            ...
            GLsizei log_length;
            host_functions.glGetProgramiv(glprogram->id, GL_INFO_LOG_LENGTH, &log_length);
            DBG(printf("Linker error length: %i\n", log_length));
            if(log_length != 0) {
               
               GLchar log_chars[log_length];
               host_functions.glGetProgramInfoLog(glprogram->id, log_length, &log_length, log_chars);
               printf("%s\n", log_chars);
            }
            // should DBG the linker error?
...
GLvoid COBALT_API cobalt_glDeleteObject(GLhandleARB obj) {
    ...
    if(glprogram)
        cobalt_glDeleteProgram(obj);
    else
        cobalt_glDeleteShader(obj);
}
...
GLvoid COBALT_API cobalt_glGetObjectParameterfv(GLhandleARB obj, GLenum pname, GLfloat *params) {
    ...
    GLint p[4];
    if(glprogram)
        cobalt_glGetProgramiv(obj, pname, p);
    else
        cobalt_glGetShaderiv(obj, pname, p);
...
GLvoid COBALT_API cobalt_glGetObjectParameteriv(GLhandleARB obj, GLenum pname, GLint *params) {
    ...
    if(glprogram)
        cobalt_glGetProgramiv(obj, pname, params);
    else
        cobalt_glGetShaderiv(obj, pname, params);
...
GLvoid COBALT_API cobalt_glGetInfoLog(GLhandleARB obj, GLsizei maxLength, GLsizei *length, GLcharARB *infoLog) {
    ...
    if(glprogram)
        cobalt_glGetProgramInfoLog(obj, maxLength, length, infoLog);
    else
        cobalt_glGetShaderInfoLog(obj, maxLength, length, infoLog);
}

GLvoid COBALT_API cobalt_glGetAttachedObjects(GLhandleARB containerObj, GLsizei maxCount, GLsizei *count, GLhandleARB *obj) {
    cobalt_glGetAttachedShaders(containerObj, maxCount, count, obj);
}
