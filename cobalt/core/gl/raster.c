...
void COBALT_API cobalt_glRasterPos3f(GLfloat x, GLfloat y, GLfloat z) {
	...
	cobalt_glGetFloatv(GL_PROJECTION_MATRIX, glmatrix);
	matrix_transpose(glmatrix, projection);
	cobalt_glGetFloatv(GL_MODELVIEW_MATRIX, glmatrix);
...
void COBALT_API cobalt_glViewport(GLint x, GLint y, GLsizei width, GLsizei height) {
	if (glstate->fbo.current_fb->id != 0) {
    	gltexture_t *tex = cobalt_getTexture(glstate->fbo.current_fb->t_color[0], glstate->fbo.current_fb->color[0]);
		...
		glstate->raster.viewport.height!=height) 
	{
		...
		glstate->raster.viewport.height = height;
#if !defined(NO_EGL) && !defined(NOX11)
		if(!g_cobalt.usefbo && !g_cobalt.usefb && glstate->fbo.fbo_draw->id==0) {
...
//printf("COBALT: glPixelTransferf(%04x, %f)\n", pname, param);
	switch(pname) {
		case GL_RED_SCALE:
		...
		case GL_MAP_COLOR:
			...
			break;
		/*default:
			printf("COBALT: stubbed glPixelTransferf(%04x, %f)\n", pname, param);*/
	// the other...
	}
}
...
void raster_to_texture(rasterlist_t *r)
{
    ...
    cobalt_glPushAttrib(GL_TEXTURE_BIT | GL_ENABLE_BIT );
	GLuint old_tex_unit, old_tex;
	old_tex_unit = glstate->texture.active;
	if (old_tex_unit!=0) cobalt_glActiveTexture(GL_TEXTURE0);
	old_tex = glstate->texture.bound[0][ENABLED_TEX2D]->glname;

	cobalt_glPixelStorei(GL_PACK_ALIGNMENT, 1);
	...
	cobalt_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	
	if(r->texture==0) {
		cobalt_glEnable(GL_TEXTURE_2D);
		cobalt_glGenTextures(1, &r->texture);
		cobalt_glBindTexture(GL_TEXTURE_2D, r->texture);

		cobalt_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		cobalt_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		cobalt_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		cobalt_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		cobalt_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0); // this is to be sure texture is not npot'ed ...
		cobalt_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0);	// ... if not needed
		cobalt_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.raster_nwidth, glstate->raster.raster_nheight,
			0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	} else {
		cobalt_glBindTexture(GL_TEXTURE_2D, r->texture);
	}
	cobalt_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.raster_width, glstate->raster.raster_height,
	...
	cobalt_glBindTexture(GL_TEXTURE_2D, old_tex);
	if (old_tex_unit!=0) 
		cobalt_glActiveTexture(old_tex_unit+GL_TEXTURE0);
	cobalt_glPopAttrib();
...
void bitmap_flush() {
	...
	if(IS_TEX1D(old_active)) cobalt_glDisable(GL_TEXTURE_1D);
	if(!IS_TEX2D(old_active)) cobalt_glEnable(GL_TEXTURE_2D);
	if(IS_TEX3D(old_active)) cobalt_glDisable(GL_TEXTURE_3D);
	if(IS_TEXTURE_RECTANGLE(old_active)) cobalt_glDisable(GL_TEXTURE_RECTANGLE_ARB);
	if(IS_CUBE_MAP(old_active)) cobalt_glDisable(GL_TEXTURE_CUBE_MAP);

	if(!glstate->raster.bm_texture) {
		cobalt_glGenTextures(1, &glstate->raster.bm_texture);
		cobalt_glBindTexture(GL_TEXTURE_2D, glstate->raster.bm_texture);
	...
	if (glstate->raster.bm_tnwidth < glstate->raster.bm_width || glstate->raster.bm_tnheight < glstate->raster.bm_height) {
		...
		cobalt_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.bm_tnwidth, glstate->raster.bm_tnheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	}
	...
	if(sx==0 && sy==0 && ex==glstate->raster.bm_width && ey==glstate->raster.bm_height) {
		cobalt_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.bm_width, glstate->raster.bm_height, GL_RGBA, GL_UNSIGNED_BYTE, glstate->raster.bitmap);
	} else {
		int alloc = 4*ex*ey;
		cobalt_scratch(alloc);
		for (int i=0; i<ey; i++)
			memcpy((char*)glstate->scratch+4*i*ex, (char*)glstate->raster.bitmap+4*(sx+(sy+i)*glstate->raster.bm_width), ex*4);
		cobalt_glTexSubImage2D(GL_TEXTURE_2D, 0, sx, sy, ex, ey, GL_RGBA, GL_UNSIGNED_BYTE, glstate->scratch);
	}
	...
	if(IS_TEX1D(old_active)) cobalt_glEnable(GL_TEXTURE_1D);
	if(!IS_TEX2D(old_active)) cobalt_glDisable(GL_TEXTURE_2D);
	if(IS_TEX3D(old_active)) cobalt_glEnable(GL_TEXTURE_3D);
	if(IS_TEXTURE_RECTANGLE(old_active)) cobalt_glEnable(GL_TEXTURE_RECTANGLE_ARB);
	if(IS_CUBE_MAP(old_active)) cobalt_glEnable(GL_TEXTURE_CUBE_MAP);

	if (old_tex!=glstate->raster.bm_texture)
		cobalt_glBindTexture(GL_TEXTURE_2D, old_tex);

	if(old_tex_unit)
		cobalt_glActiveTexture(GL_TEXTURE0 + old_tex_unit);
}
...
void COBALT_API cobalt_glDrawPixels(GLsizei width, GLsizei height, GLenum format,
	...
	if(type==GL_BITMAP) {
		cobalt_glBitmap(width, height, 0, 0, 0, 0, data);
		return;
	}
	...
	if (glstate->list.active) {
        ...
        *r->shared = 0;
		...
		if(r->texture && (width>r->nwidth || height>r->nheight)) {
			cobalt_glDeleteTextures(1, &r->texture);
			r->texture = 0;
		}
	}
	raster_to_texture(r);
