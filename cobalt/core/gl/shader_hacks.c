...
static const hack_t cobalt_hacks[] = {
...
// For Stellaris
static const char* cobalt_sign_1[] = {
"if (Data.Type == 1)",
"if (Data.BlendMode == 0)",
};
static const char* cobalt_hacks_1[] = {
"if (Data.Type == 1)",
"if (Data.Type == 1.0)",
...
// For Psychonauts
static const char* cobalt_sign_2[] = {
...
static const char* cobalt_sign_2_main = 
"void main()\n"
"{\n";


static const char* cobalt_hacks_2_1[] = {
"vec4 ps_t3;",
"vec4 ps_t2;",
"vec4 ps_t1;",
"vec4 ps_t0;",
...
static const char* cobalt_hacks_2_2[] = {
"\tps_t3 = gl_TexCoord[3];",
"\tps_t2 = gl_TexCoord[2];",
"\tps_t1 = gl_TexCoord[1];",
"\tps_t0 = gl_TexCoord[0];",
...
static char* ShaderHacks_1(char* shader, char* Tmp, int* tmpsize)
{
    ...
    for (int i=0; i<sizeof(cobalt_sign_1)/sizeof(cobalt_sign_1[0]); i++)
        if(!strstr(Tmp, cobalt_sign_1[i]))
            return Tmp;
    // Do the replace
    for (int i=0; i<sizeof(cobalt_hacks_1)/sizeof(cobalt_hacks_1[0]); i+=2)
        if(strstr(Tmp, cobalt_hacks_1[i])) {
            if(Tmp==shader) {Tmp = (char*)malloc(*tmpsize); strcpy(Tmp, shader);}   // hacking!
            Tmp = cobalt_inplace_replace_simple(Tmp, tmpsize, cobalt_hacks_1[i], cobalt_hacks_1[i+1]);
        }
    return Tmp;
}
...
{
    char* p = strstr(Tmp, cobalt_sign_2[i]);
    if(!p) return Tmp;  // not found
    char* m = strstr(Tmp, cobalt_sign_2_main);
    ...
    if(Tmp==shader) {Tmp = (char*)malloc(*tmpsize); strcpy(Tmp, shader); m = strstr(Tmp, cobalt_sign_2_main);}   // hacking!
    m += strlen(cobalt_sign_2_main);
    Tmp = cobalt_inplace_insert(m, cobalt_hacks_2_2[i], Tmp, tmpsize);
    Tmp = cobalt_inplace_replace_simple(Tmp, tmpsize, cobalt_sign_2[i], cobalt_hacks_2_1[i]);
    return Tmp;
}
...
static char* ShaderHacks_2(char* shader, char* Tmp, int* tmpsize)
{
    ...
    for (int i=0; i<sizeof(cobalt_sign_2)/sizeof(cobalt_sign_2[0]); i++)
...
char* ShaderHacks(char* shader)
{
    ...
    // generic
    for (int i=0; i<sizeof(cobalt_hacks)/sizeof(cobalt_hacks[0]); ++i) {
        char* f = cobalt_hacks[i].sign;
        int n = cobalt_hacks[i].n;
        if(strstr(Tmp, f)) {
            if(Tmp==shader) {Tmp = (char*)malloc(tmpsize); strcpy(Tmp, shader);}   // hacking!
            for (int j=0; j<n; j+=2) {
                if(j) f = cobalt_hacks[i].next[j-1];
                Tmp = cobalt_inplace_replace_simple(Tmp, &tmpsize, f, cobalt_hacks[i].next[j]);
            }
        }
    }
    return Tmp;
