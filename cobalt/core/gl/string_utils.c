...
char* cobalt_inplace_replace(char* pBuffer, int* size, const char* S, const char* D)
{
    ...
    pBuffer = cobalt_resize_if_needed(pBuffer, size, (lD-lS)*cobalt_count_string(pBuffer, S));
    char* p = pBuffer;
    while((p = strstr(p, S)))
    {
...
{
    //COBALT_LOGD("BY INDEX: %s", replacement);
    //COBALT_LOGD("BY INDEX: %i", strlen(replacement));
...
char * InplaceInsertByIndex(char * source, int *sourceLength, const int insertPoint, const char *insertedString){
    ...
    source = cobalt_resize_if_needed(source, sourceLength, insertLength);
...
{
    char* m = cobalt_resize_if_needed(master, size, strlen(S));
    if(m!=master) {
        pBuffer += (m-master);
        master = m;
    }
...
int cobalt_getline_for(const char* pBuffer, const char* S)
{
    int n=0;
    const char* p = pBuffer;
    const char* end = cobalt_find_string(pBuffer, S);
    if(!end)
        return 0;
    while((p=strstr(p, "\n"))) {
        p+=strlen("\n");
...
char* cobalt_append(char* pBuffer, int* size, const char* S) {
    char* p =pBuffer;
    p = cobalt_resize_if_needed(pBuffer, size, strlen(S));
    strcat(p, S);
    return p;
}

static int cobalt_is_blank(char c)  {
    switch(c) {
        case ' ':
        case '\t':
        case '\n':
...
char* cobalt_str_next(char *pBuffer, const char* S) {
    if(!pBuffer) return NULL;
    char *p = strstr(pBuffer, S);
    return (p)?p:(p+strlen(S));
}

char* cobalt_next_str(char* pBuffer) {
    if(!pBuffer) return NULL;
    while(cobalt_is_blank(*pBuffer))
        ++pBuffer;
    return pBuffer;
}

char* cobalt_prev_str(char* Str, char* pBuffer) {
    if(!pBuffer) return NULL;
    if(pBuffer == Str)
        return Str;
    ...
    do {
        --pBuffer;
    } while(cobalt_is_blank(*pBuffer) && (pBuffer!=Str));
    // go to blank
    while((pBuffer!=Str) && !cobalt_is_blank(*(pBuffer-1)))
        --pBuffer;
    return pBuffer;
}

char* cobalt_next_blank(char* pBuffer) {
    if(!pBuffer) return NULL;
    while(!cobalt_is_blank(*pBuffer))
        ++pBuffer;
    return pBuffer;
...
char* cobalt_next_line(char* pBuffer) {
    if(!pBuffer) return NULL;
...
const char* cobalt_get_next_str(char* pBuffer) {
    static char buff[100] = {0};
    buff[0] = '\0';
    if(!pBuffer) return NULL;
    char* p1 = cobalt_next_str(pBuffer);
    if(!p1) return buff;
    char* p2 = cobalt_next_blank(p1);
    if(!p2) return buff;
    int i=0;
    while(p1!=p2 && i<99)
        buff[i++] = *(p1++);
...
{
    int lS = strlen(S), lD = strlen(D);
    pBuffer = cobalt_resize_if_needed(pBuffer, size, (lD-lS)*cobalt_countstring_simple(pBuffer, S));
    char* p = pBuffer;
    while((p = strstr(p, S)))
    {
        // found an occurrence of S
