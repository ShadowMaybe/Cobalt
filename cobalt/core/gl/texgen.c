...
void COBALT_API cobalt_glTexGenfv(GLenum coord, GLenum pname, const GLfloat *param) {
...
/*    if (!norm) {
        printf("COBALT: GL_SPHERE_MAP without Normals\n");
        return;
    }*/
...
void reflection_loop(const GLfloat *verts, const GLfloat *norm, GLfloat *out, GLint count, GLushort *indices) {
    // based on https://www.opengl.org/wiki/Mathematics_of_glTexGen
/*    if (!norm) {
        printf("COBALT: GL_REFLECTION_MAP without Normals\n");
        return;
    }*/
    GLfloat InvModelview[16];
...
static inline void tex_coord_loop(GLfloat *verts, GLfloat *norm, GLfloat *out, GLint count, GLenum type, GLfloat *param_o, GLfloat *param_e, GLushort *indices) {
    switch (type) {
        ...
        case GL_EYE_LINEAR:
            ...
            break;
        case GL_SPHERE_MAP:
            //printf("COBALT: GL_SPHERE_MAP with only 1 TexGen available");  //Broken here
            break;
    }
}
     ...
     && (glstate->enable.texgen_r[texture] && (glstate->texgen[texture].R==GL_REFLECTION_MAP)))
    {
        if(hardext.cubemap) {
            *needclean=1;
            ...
            if (old_tex!=texture) cobalt_glActiveTexture(GL_TEXTURE0 + texture);
            realize_active();
            ...
            if (!(g_cobalt.texmat || glstate->texture_matrix[texture]->identity)) {
                
                GLenum old_mat = glstate->matrix_mode;
                if(old_mat!=GL_TEXTURE) cobalt_glMatrixMode(GL_TEXTURE);
                host_functions.glLoadMatrixf(getTexMat(texture));
                if(old_mat!=GL_TEXTURE) cobalt_glMatrixMode(old_mat);
            }

            if (old_tex!=texture) cobalt_glActiveTexture(GL_TEXTURE0 + old_tex);
        } else {
            if (!IS_TEX2D(glstate->enable.texture[texture]))
                return;
            if ((*coords)==NULL) 
...
void gen_tex_clean(GLint cleancode, int texture) {
	...
	if (cleancode == 1) {
        ...
        // check Texture Matrix
        if ((hardext.esversion==1) && !(g_cobalt.texmat || glstate->texture_matrix[texture]->identity)) {
...
void COBALT_API cobalt_glLoadTransposeMatrixd(const GLdouble *m) {
	...
	for (int i=0; i<16; i++)
		mf[i] = m[i];
	cobalt_glLoadTransposeMatrixf(mf);
}
