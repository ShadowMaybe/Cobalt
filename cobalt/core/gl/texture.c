    ...
    switch(internalformat) {
        ...
        case GL_RGB32F:
            ...
            *type = (hardext.floattex)?GL_FLOAT:GL_UNSIGNED_BYTE;
            break;
        default:
            printf("COBALT: Warning, unknown Internalformat (%s)\n", PrintEnum(internalformat));
            *format = GL_RGBA;
            *type = GL_UNSIGNED_BYTE;
            break;
    }
    ...
    if(*format != intermediaryformat || intermediaryformat!=internalformat) {
        ...
        switch (*type) {
            ...
            case GL_UNSIGNED_BYTE:
                if(dest_format==GL_RGB && g_cobalt.avoid24bits) {
                    dest_format = GL_RGBA;
                    convert = 1;
                }
                break;
    ...
    if (data) {
        if (convert) {
            ...
            if (! pixel_convert(data, &pixels, width, height,
                ...
                printf("COBALT: swizzle error: (%s, %s -> %s, %s)\n",
            ...
            *format = dest_format;
            ...
            if (raster_need_transform())
                if (!pixel_transform(data, &pixels, width, height,
                    ...
                    printf("COBALT: swizzle/convert error: (%s, %s -> %s, %s)\n",
                        PrintEnum(*format), PrintEnum(*type), PrintEnum(dest_format), PrintEnum(dest_type));
                    pix2 = pixels;
                }
...
GLenum swizzle_internalformat(GLenum *internalformat, GLenum format, GLenum type) {
    ...
    switch(*internalformat) {
        ...
        case GL_RGB5:
            ...
            break;
        case GL_RGB:
             if(g_cobalt.avoid16bits==0 && format==GL_RGB && type==GL_UNSIGNED_SHORT_5_6_5) {
                sret = ret = GL_RGB5;
                break;
            }
        case GL_RGB8:
        ...
        case GL_RGB5_A1:
            ...
            break;
        case GL_RGBA:
            if(g_cobalt.avoid16bits==0 && format==GL_RGBA && type==GL_UNSIGNED_SHORT_5_5_5_1) {
                sret = ret = GL_RGB5_A1;
                break;
            }
            if(g_cobalt.avoid16bits==0 && format==GL_RGBA && type==GL_UNSIGNED_SHORT_4_4_4_4) {
                sret = ret = GL_RGBA4;
                break;
            }
...
int wrap_npot(GLenum wrap) {
    switch(wrap) {
        case 0: return (g_cobalt.defaultwrap)?1:0;
        case GL_CLAMP:
...
void COBALT_API cobalt_glTexImage2D(GLenum target, GLint level, GLint internalformat,
    ...
    if(bound->binded_fbo && (bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_STENCIL_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT))
    {
        ...
        if(data!=NULL) 
            printf("COBALT: Warning, Depth/stencil texture resized and with data\n");
        // get new size...
        ...
        if(bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT)
        {
            if(bound->renderdepth) {
                cobalt_glBindRenderbuffer(GL_RENDERBUFFER, bound->renderdepth);
                cobalt_glRenderbufferStorage(GL_RENDERBUFFER, (bound->binded_attachment==GL_DEPTH_ATTACHMENT)?GL_DEPTH_COMPONENT16:GL_DEPTH24_STENCIL8, nwidth, nheight);
        ...
        {
            cobalt_glBindRenderbuffer(GL_RENDERBUFFER, bound->renderstencil);
            cobalt_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, nwidth, nheight);
            cobalt_glBindRenderbuffer(GL_RENDERBUFFER, 0);
        }
        // all done, exit
        errorGL();
    ...
    if (g_cobalt.automipmap) {
        if (level>0)
            if ((g_cobalt.automipmap==1) || (g_cobalt.automipmap==3) || bound->mipmap_need) {
                return;			// has been handled by auto_mipmap
            }
            else if(g_cobalt.automipmap==2)
                bound->mipmap_need = 1;
    }
    if(level>0 && (bound->npot && g_cobalt.forcenpot))
        return;         // no mipmap...
...
#ifdef TEXSTREAM
        if (g_cobalt.texstream && (target==GL_TEXTURE_2D || target==GL_TEXTURE_RECTANGLE_ARB) && (width>=256 && height>=224) && 
        ((internalformat==GL_RGB) || (internalformat==3) || (internalformat==GL_RGB8) || (internalformat==GL_BGR) || (internalformat==GL_RGB5) || (internalformat==GL_RGB565)) || (g_cobalt.texstream==2) ) {
        ...
        if (bound->shrink!=0) {
            ...
            case 11:
                if ((mipwidth > 1) && (mipheight > 1)) {
                    ...
                    if(g_cobalt.texshrink==11 && newwidth>hardext.maxsize) newwidth=hardext.maxsize; // in case of some rounding error
                    if(g_cobalt.texshrink==11 && newheight>hardext.maxsize) newheight=hardext.maxsize; // in case of some rounding error
                    width = newwidth;
                    height = newheight;
                    bound->shrink = 1;
                }
        ...
        if(bound->npot) {
            if(target==GL_TEXTURE_RECTANGLE_ARB && hardext.npot)
                limitednpot=1;
                 ...
                 || (g_cobalt.automipmap==3) 
                 || (g_cobalt.automipmap==4 && width!=height) 
...
#define NO_1x1
        ...
        if ((g_cobalt.automipmap==4) && (nwidth!=nheight))
            bound->mipmap_auto = 0;
        ...
        if (!(g_cobalt.texstream && bound->streamed)) {
            if ((target!=GL_TEXTURE_RECTANGLE_ARB) && (g_cobalt.automipmap!=3) && (bound->mipmap_need || bound->mipmap_auto) && !(bound->npot && hardext.npot<2) && (bound->max_level==-1)) {
                ...
                && (bound->mipmap_need || g_cobalt.automipmap==3)) {
                    // remove the need for mipmap...
                    bound->mipmap_need = 0;
                    cobalt_glTexParameteri(rtarget, GL_TEXTURE_MIN_FILTER, bound->sampler.min_filter); // forcing min_filter to be recomputed
                }
            }
            ...
            if(g_cobalt.automipmap==5 && !level)
                bound->mipmap_done = 0;
            // check if max_level is set... and calculate higher level mipmap
            if(((bound->max_level == level && (level || bound->mipmap_need)) || (callgeneratemipmap && level==0) || (g_cobalt.automipmap==5 && level && !bound->mipmap_done)) && !(bound->max_level==bound->base_level && bound->max_level==0)) {
                if(g_cobalt.automipmap==5 && level==1)
                    bound->mipmap_done = 1;
        ...
        /*if (bound && bound->mipmap_need && !bound->mipmap_auto && (g_cobalt.automipmap!=3))
            host_functions.glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/
        } else {
            if (pixels)
                cobalt_glTexSubImage2D(rtarget, level, 0, 0, width, height, format, type, pixels);	// (should never happens) updload the 1st data...
        }
    ...
    if ((target==GL_TEXTURE_2D) && g_cobalt.texcopydata && ((g_cobalt.texstream && !bound->streamed) || !g_cobalt.texstream)) {
        if (bound->data) 
        ...
        if (datab) {
            if (!pixel_convert(pixels, &bound->data, width, height, format, type, GL_RGBA, GL_UNSIGNED_BYTE, 0, glstate->texture.unpack_align))
                printf("COBALT: Error on pixel_convert when TEXCOPY in glTexImage2D\n");
        } else {
...
void COBALT_API cobalt_glTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset,
    ...
    if (g_cobalt.texstream && (bound->streamed)) {
        ...
        if (! pixel_convert(old, &tmp, width, height,
            ...
            printf("COBALT: swizzle error: (%#4x, %#4x -> GL_RGB, UNSIGNED_SHORT_5_6_5)\n",
                        format, type);
        }
        format = GL_RGB;
        type = GL_UNSIGNED_SHORT_5_6_5;
...
#endif
    {
        if (!pixel_convert(old, &pixels, width, height, format, type, bound->inter_format, bound->inter_type, 0, glstate->texture.unpack_align)) {
            printf("COBALT: Error in pixel_convert while glTexSubImage2D\n");
        } else {
...
/*	// copy the texture to the buffer
    ...
    for (int yy=0; yy<height; yy++) {
        ...
        if((bound->max_level==bound->base_level) && (bound->base_level==0))
            genmipmap = 0;
        if(genmipmap && (g_cobalt.automipmap!=3)) {
            int leveln = level, nw = width, nh = height, xx=xoffset, yy=yoffset;
            void *ndata = pixels;
            while(nw!=1 || nh!=1) {
                if(pixels) {
    ...
    /*if (bound->mipmap_need && !bound->mipmap_auto && (g_cobalt.automipmap!=3) && (!g_cobalt.texstream || (g_cobalt.texstream && !bound->streamed)))
        host_functions.glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/
    ...
    //printf("*texcopy* glTexSubImage2D, xy=%i,%i, size=%i,%i=>%i,%i, format=%s, type=%s, tex=%u\n", xoffset, yoffset, width, height, bound->width, bound->height, PrintEnum(format), PrintEnum(type), bound->glname);
        ...
        if (!pixel_convert(pixels, &tmp, width, height, format, type, GL_RGBA, GL_UNSIGNED_BYTE, bound->width, glstate->texture.unpack_align))
            printf("COBALT: Error on pixel_convert while TEXCOPY in glTexSubImage2D\n");
    }
...
// 1d stubs
void COBALT_API cobalt_glTexImage1D(GLenum target, GLint level, GLint internalFormat,
                  GLsizei width, GLint border,
                  GLenum format, GLenum type, const GLvoid *data) {

    // TODO: maybe too naive to force GL_TEXTURE_2D here?
    cobalt_glTexImage2D(GL_TEXTURE_1D, level, internalFormat, width, 1,
                 border, format, type, data);
}
void COBALT_API cobalt_glTexSubImage1D(GLenum target, GLint level, GLint xoffset,
                     GLsizei width, GLenum format, GLenum type,
                     const GLvoid *data) {

    cobalt_glTexSubImage2D(GL_TEXTURE_1D, level, xoffset, 0,
...
void COBALT_API cobalt_glTexStorage1D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei width)
{
    DBG(printf("glTexStorage1D(%s, %d, %s, %d)\n", PrintEnum(target), levels, PrintEnum(internalformat), width);)
    cobalt_glTexImage1D(target, 0, internalformat, width, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
}
void COBALT_API cobalt_glTexStorage2D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei width, GLsizei height)
{
    ...
    if((internalformat==GL_COMPRESSED_RGB_S3TC_DXT1_EXT || internalformat==GL_COMPRESSED_SRGB_S3TC_DXT1_EXT) 
     && !g_cobalt.avoid16bits)
        cobalt_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, NULL);
    else if(((internalformat==GL_COMPRESSED_RGBA_S3TC_DXT1_EXT || internalformat==GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT)) 
     && !g_cobalt.avoid16bits)
        cobalt_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, NULL);
     ...
     && !g_cobalt.avoid16bits)
        cobalt_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, NULL);
    else
        cobalt_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    ...
    if(levels>1 && isDXTc(internalformat)) {
        ...
        for (int i=1; i<=mlevel; ++i)
            cobalt_glTexImage2D(target, i, internalformat, nlevel(width, i), nlevel(height, i), 0, bound->format, bound->type, NULL);
        noerrorShim();
        return;
    }
    ...
    if(mlevel>levels-1) {
        ...
        if(levels>1 && g_cobalt.automipmap!=3)
            bound->mipmap_need = 1;
    }
