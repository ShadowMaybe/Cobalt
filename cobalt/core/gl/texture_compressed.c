                            ...
                            GLsizei imageSize, const GLvoid *data) 
{
    ...
    if(level && (g_cobalt.automipmap==3)) {
        noerrorShim();
    ...
    if (isDXTc(internalformat)) {
        ...
        if(g_cobalt.nodownsampling==1) {  // will be removed soon, avoid16bits is better
            format = GL_RGBA;
        ...
        if (datab) {
            ...
            half=pixels;
            if(!g_cobalt.nodownsampling && !g_cobalt.avoid16bits) {
        ...
        int oldalign;
        cobalt_glGetIntegerv(GL_UNPACK_ALIGNMENT, &oldalign);
        if (oldalign!=1) 
        ...
        cobalt_glTexImage2D(target, level, new_intformat, width, height, border, format, type, half);
        ...
        if(level) {
            ...
            while(nww!=1 || nhh!=1) {
                ...
                ++leveln;
                cobalt_glTexImage2D(target, leveln, new_intformat, nww, nhh, border, format, type, out);
                if(out!=ndata)
                    free(out);
            }
            bound->mipmap_auto = 1;
        ...
        if (oldalign!=1) 
            cobalt_glPixelStorei(GL_UNPACK_ALIGNMENT, oldalign);
        if (half!=pixels)
            free(half);
        if (pixels!=datab)
            free(pixels);
                               ...
                               GLsizei imageSize, const GLvoid *data) 
{
    ...
    if (isDXTc(format)) {
        ...
        int oldalign;
        cobalt_glGetIntegerv(GL_UNPACK_ALIGNMENT, &oldalign);
        if (oldalign!=1) cobalt_glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        cobalt_glTexSubImage2D(target, level, xoffset/2, yoffset/2, width/2, height/2, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, half);
        if (oldalign!=1) cobalt_glPixelStorei(GL_UNPACK_ALIGNMENT, oldalign);
        #else
        ...
        cobalt_glTexSubImage2D(target, level, xoffset, yoffset, width, height, GL_RGBA, GL_UNSIGNED_BYTE, half);
        #endif
        if (half!=pixels)
            free(half);
        if (pixels!=datab)
...
void COBALT_API cobalt_glGetCompressedTexImage(GLenum target, GLint lod, GLvoid *img) {
    ...
    cobalt_glGetTexImage(target, lod, GL_RGBA, GL_UNSIGNED_BYTE, (GLvoid*)src);
...
void COBALT_API cobalt_glCompressedTexImage1D(GLenum target, GLint level, GLenum internalformat,
    ...
    cobalt_glCompressedTexImage2D(target, level, internalformat, width, 1, border, imageSize, data);
}
...
void COBALT_API cobalt_glCompressedTexImage3D(GLenum target, GLint level, GLenum internalformat,
    ...
    cobalt_glCompressedTexImage2D(target, level, internalformat, width, height, border, imageSize, data);
}
...
void COBALT_API cobalt_glCompressedTexSubImage1D(GLenum target, GLint level, GLint xoffset,
    ...
    cobalt_glCompressedTexSubImage2D(target, level, xoffset, 0, width, 1, format, imageSize, data);
}
void COBALT_API cobalt_glCompressedTexSubImage3D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset,
    ...
    cobalt_glCompressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, imageSize, data);
}
