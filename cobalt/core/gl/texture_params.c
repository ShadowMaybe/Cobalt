...
int inline tex_setup_needchange(GLuint itarget) {
    ...
    if ((itarget == ENABLED_TEXTURE_RECTANGLE) 
        ...
        || (hardext.esversion==1 && !g_cobalt.texmat && !glstate->texture_matrix[texunit]->identity)
        )
        return 1;
    return 0;
}
void tex_setup_texcoord(GLuint len, int changes, GLuint itarget, vertexattrib_t* ptr) {
    ...
    if (changes) {
        ...
        if (!(g_cobalt.texmat || glstate->texture_matrix[texunit]->identity))
...
gltexture_t* cobalt_getTexture(GLenum target, GLuint texture) {
    ...
    if (k == kh_end(list)){
        ...
        tex->mipmap_auto = (g_cobalt.automipmap==1);
        tex->mipmap_need = (g_cobalt.automipmap==1)?1:0;
        tex->base_level = -1;
        tex->max_level = -1;
        tex->streamingID = -1;
...
void COBALT_API cobalt_glBindTexture(GLenum target, GLuint texture) {
    ...
    if ((target!=GL_PROXY_TEXTURE_2D) && glstate->list.compiling && glstate->list.active && !glstate->list.pending) {
        ...
        tex = cobalt_getTexture(target, texture);
...
GLenum get_texture_min_filter(gltexture_t* texture, glsampler_t* sampler)
{
    ...
    || ((g_cobalt.automipmap==1) && (texture->mipmap_auto==0)) 
    ...
    if(texture->valid && (texture->npot && g_cobalt.forcenpot)) {
...
GLenum get_texture_wrap(GLenum wrap, gltexture_t* texture)
{
    ...
    case GL_MIRRORED_REPEAT_OES:
        if(g_cobalt.defaultwrap==2 && hardext.npot<3 && !texture->valid)
            wrap = GL_CLAMP_TO_EDGE;
            ...
            case GL_GENERATE_MIPMAP:
                ...
                if(hardext.esversion>1) {
                    /*if(texture->valid) {
                        ...
                        cobalt_glGenerateMipmap(rtarget);
                    }*/
                    return;
                }
...
void COBALT_API cobalt_glTexParameteriv(GLenum target, GLenum pname, const GLint * params) {
    ...
    if(pname==GL_TEXTURE_BORDER_COLOR)
        for(int i=1; i<4; ++i)
            fparams[i] = params[i];
    cobalt_glTexParameterfv(target, pname, fparams);
}
    ...
    if (list) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (k != kh_end(list)) {
                ...
                if(tex->binded_fbo) {
                    if(tex->renderdepth)
                        cobalt_glDeleteRenderbuffers(1, &tex->renderdepth);
                    if(tex->renderstencil)
                        cobalt_glDeleteRenderbuffers(1, &tex->renderstencil);
                }
                errorGL();
#ifdef TEXSTREAM
                if (g_cobalt.texstream && tex->streamed)
...
void realize_bound(int TMU, GLenum target) {
    ...
    switch (target) {
        ...
        case GL_TEXTURE_RECTANGLE_ARB:
            ...
            if (g_cobalt.texstream && (streamingID>-1)) {
...
void realize_textures(int drawing) {
    ...
    for (int i=0; i<glstate->bound_changed; i++) {
        ...
        if(drawing) {
            if((g_cobalt.automipmap==3) || ((g_cobalt.automipmap==1) && (tex->mipmap_auto==0)) || (tex->compressed && (tex->mipmap_auto==0)))
                tex->mipmap_need = 0;
            else
