...
#define SHUT(a) if(!g_cobalt.nobanner) a
...
static int init_drm_and_gbm()
{
    ...
    drm_fd = open(g_cobalt.drmcard, O_RDWR | O_CLOEXEC);
    ...
    if(!resources) {
        printf("COBALT: Error initializing drm resources\n");
        return 0;
    }
    ...
    if (!connector) {
         ...
         * a connector..
         */
        printf("COBALT: DRM no connected connector!\n");
        return 0;
    }
    ...
    if (!drm_mode) {
        printf("COBALT: DRM could not find mode!\n");
        return 0;
    }
    /* find encoder: */
    ...
    if (encoder) {
        ...
        if (crtc_id == 0) {
            printf("COBALT: DRM no crtc found!\n");
            return 0;
        }
    ...
    if(!gbmdev) {
        printf("COBALT: Error initializing gbm device\n");
        return 0;
    }
    ...
    if(!gbmsurf) {
        printf("COBALT: Error initializing gbm surface\n");
        return 0;
    }
...
static int OpenGBM()
{
    ...
    SHUT(printf("COBALT: GBM on card %s is %s\n", g_cobalt.drmcard, available?"Available":"Not available"));
...
void LoadGBMFunctions()
{
    ...
    #define GBMFUNC(ret, name, args) \
        ...
        if(!gbmdrm_##name) { \
            printf("COBALT: libgbm function %s missing, no gbm surface enabled\n", #name); \
            CancelGBM(); \
            return; \
        }
    #include "gbmfunc.h"
    ...
    #define DRMFUNC(ret, name, args) \
        ...
        if(!gbmdrm_##name) { \
            printf("COBALT: libdrm function %s missing, no gbm surface enabled\n", #name); \
            CancelGBM(); \
            return; \
        }
    #include "drmfunc.h"
...
void* CreateGBMWindow(int w, int h)
{
    void* ret = gbmsurf;
    if(!ret)
        printf("COBALT: Warning, cannot create gbm surface %dx%d\n", w, h);
}
...
int FindGBMConfig(EGLDisplay eglDisp, EGLConfig *configs, int numFounds)
{
    ...
    printf("COBALT: Warning, no EGLConfig matching GBM Format found\n");
    return 0;   // not found...
}
EGLBoolean GBMMakeCurrent(EGLDisplay eglDisp, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    if (r) {
        printf("COBALT: GBM failed to set mode: %s\n", strerror(errno));
        return EGL_FALSE;
    }
