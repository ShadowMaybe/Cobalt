...
void glx_getMainFBSize(GLint* width, GLint* height) {
    ...
    DBG(printf("cobalt_getMainFBSize() %dx%d -> ", *width, *height);)
...
// Generic create native window to use with "COBALT_FB=1" (so with EGL_DEFAULT_DISPLAY and without X11)
static void* create_native_window(int w, int h) {
#if !defined(ANDROID) && !defined(AMIGAOS4)
    if(bcm_host) return create_rpi_window(w, h);
#endif
#ifndef NO_GBM
    if(g_cobalt.usegbm) return CreateGBMWindow(w, h);
#endif
    return NULL;
}
...
#endif
#ifndef NO_GBM
    if(g_cobalt.usegbm) return DeleteGBMWindow(win);
#endif
}
...
static void RecycleAddSurface(GLXDrawable drawable, SharedEGLSurface_t* surf) {
    ...
    DBG(printf("COBALT: EGLSurface for drawable %p Added\n", (void*)drawable);)
}
...
static SharedEGLSurface_t* RecycleGetSurface(GLXDrawable drawable) {
    ...
    if (k != kh_end(eglsurfaces)){
        DBG(printf("COBALT: EGLSurface for drawable %p found\n", (void*)drawable);)
        return kh_value(eglsurfaces, k);
    }
    if((g_cobalt.usefb || g_cobalt.usefbo) && kh_size(eglsurfaces)) {
...
static void RecycleDelSurface(GLXDrawable drawable) {
    ...
    if (k != kh_end(eglsurfaces)){
        ...
        DBG(printf("COBALT: EGLSurface for drawable %p removed\n", (void*)drawable);)    
    }
    return;
}
#endif
...
static void init_display(Display *display) {
    ...
    if(!eglDisplay) {
        if (g_cobalt.usefb || g_cobalt.usepbuffer) {
...
static void fill1GLXFBConfig(Display *display, EGLConfig eglConfig, int DB, GLXFBConfig fbConfig) {
    ...
    if(!fbConfig->associatedVisualId || g_cobalt.usefb || g_cobalt.usefbo || g_cobalt.usepbuffer) {
...
void pandora_set_gamma() {
     {
        char buf[50];
        if(g_cobalt.gamma>0.0f)
            sprintf(buf, "sudo /usr/pandora/scripts/op_gamma.sh %.2f", g_cobalt.gamma);
        else
...
void glx_init() {
    // init map_drawable
    int ret;
    if( !cobalt_getMainFBSize )
        cobalt_getMainFBSize = glx_getMainFBSize;
    ...
    if (g_cobalt.xrefresh || g_cobalt.stacktrace) 
    {
    ...
    init_liveinfo();
    if (sock>-1) {
        COBALT_LOGD("LiveInfo detected, fps will be shown\n");
    }
#endif
}
...
GLXContext cobalt_glXCreateContext(Display *display,
                            XVisualInfo *visual,
    ...
    if(depthBits==24 && glxfbconfig->stencilBits==8 && !(g_cobalt.usefbo || g_cobalt.usepbuffer || hardext.esversion==2))
        depthBits = 16;
    ...
    EGLint configAttribs[] = {
        ...
        EGL_SURFACE_TYPE, (g_cobalt.usegbm)?EGL_WINDOW_BIT:(g_cobalt.usepbuffer?EGL_PBUFFER_BIT:(EGL_WINDOW_BIT | EGL_PBUFFER_BIT)),
        EGL_NONE
    };
    if (g_cobalt.usefb)
        ++fbcontext_count;
    ...
    if(fake->eglConfigsCount && g_cobalt.usegbm)
...
GLXContext cobalt_glXCreateContextAttribsARB(Display *display, GLXFBConfig config,
    ...
    if(config && config->drawableType==GLX_PBUFFER_BIT) {
        ...
        EGLint configAttribs[] = {
            ...
            EGL_DEPTH_SIZE, (g_cobalt.usefb)?24:config->depthBits,
            EGL_STENCIL_SIZE, (g_cobalt.usefb)?8:config->stencilBits,
#else
            ...
            EGL_RENDERABLE_TYPE, (hardext.esversion==1)?EGL_OPENGL_ES_BIT:EGL_OPENGL_ES2_BIT,
            EGL_SURFACE_TYPE, g_cobalt.usepbuffer?EGL_PBUFFER_BIT:type,
            EGL_NONE
        };
        if (g_cobalt.usefb)
            ++fbcontext_count;
...
void cobalt_glXDestroyContext(Display *display, GLXContext ctx) {
    ...
    if (ctx->eglContext) {
        ...
        if (g_cobalt.usefbo && ctx->contextType==0) {
        ...
        ctx->eglContext = 0;
        if (ctx->eglSurface != 0) {
            if(g_cobalt.usefb!=1 || !fbcontext_count) { // (This may cause troble on Pandora, has some driver doesn't seems to like to many Creation of the surface)
				int destroySurf = 1;
        ...
        if(ctx->shared_eglsurface && (*ctx->shared_eglsurface)<=0  && !g_cobalt.glxrecycle)
...
Display *cobalt_glXGetCurrentDisplay() {
...
XVisualInfo *cobalt_glXChooseVisual(Display *display,
                             int screen,
                             int *attributes) {
...
*/  // this makes window of TokiTory transparent...
    ...
    XVisualInfo *visuals = XGetVisualInfo(display, VisualDepthMask|VisualClassMask, &xvinfo, &n);
    if (!n) {
        LOGD("Warning, cobalt_glXChooseVisual: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
        return NULL;
    }
    ...
    if(cur)
        confs = cobalt_glXChooseFBConfig(display, screen, attr, &count);
    else
        confs = cobalt_glXGetFBConfigs(display, screen, &count);
    if(!count) {
...
Bool cobalt_glXMakeCurrent(Display *display,
    ...
    if(glxContext && glxContext->glstate) {
        /*if(!context && !glxContext->doublebuff && !glxContext->contextType) {
            cobalt_glXSwapBuffers(display, glxContext->drawable);
        } else*/
            cobalt_glFlush();
    }
    ...
    if(context) {
        if(context->drawable==drawable && context->eglSurface) {
            ...
            if(created) {
                ...
                } else if((g_cobalt.usefb) || g_cobalt.usegbm) {
                    // Get size of desktop
                ...
                if(g_cobalt.usepbuffer) {
                    ...
                    if(createPBuffer(display, egl_attribs, &eglSurf, &eglCtx, &eglConfig, (depth>16)?8:5, (depth==15)?5:(depth>16)?8:6, (depth>16)?8:5, (depth==32)?8:0, context->samplebuffers, context->samples)==0) {
                        // fail too, abort
                        COBALT_LOGE("PBuffer creation failed too\n");
                        return 0;
                    }
                    int Width, Height;
...
#endif
                {
                    if(g_cobalt.usefb || g_cobalt.usefbo || g_cobalt.usegbm) {
                        if(eglSurface) // cannot create multiple eglSurface for the same Framebuffer?
                            ...
                            if(eglSurf == EGL_NO_SURFACE) {
                                ...
                                DBG(printf("COBALT: EglSurf Recycled\n");)
                            }
                            eglSurface = context->eglSurface = eglSurf;
                            if(!eglSurf) {
                                DBG(printf("COBALT: Warning, EglSurf is null\n");)
                                CheckEGLErrors();
                            }
                        }
                    } else {
    ...
    DBG(printf("COBALT: eglMakeCurrent(%p, %p, %p, %p)\n", eglDisplay, eglSurf, eglSurf, eglCtx);)
    CheckEGLErrors();
    glxContext = context;
    if(!result) {
    ...
    if (context) {
        ...
        if (result) {
            if (g_cobalt.usefbo && !created) {
                ...
                if(GetEnvVarFmt("COBALT_FBO","%dx%d",&fbo_width, &fbo_height)==2) {
                    COBALT_LOGD("Forcing FBO size %dx%d (%dx%d)\n", fbo_width, fbo_height, g_width, g_height);
                    g_width = fbo_width; 
                    g_height = fbo_height;
                }
                // create the main_fbo...
...
Bool cobalt_glXMakeContextCurrent(Display *display, int drawable,
                           int readable, GLXContext context) {
    DBG(printf("glXMakeContextCurrent(%p, %X, %X, %p)\n", display, drawable, readable, context);)
    return cobalt_glXMakeCurrent(display, drawable, context);
}

void cobalt_glXSwapBuffers(Display *display,
                    GLXDrawable drawable) {
    static int frames = 0;
...
#ifdef USE_FBIO
    if (g_cobalt.vsync && fbdev >= 0 && PBuffer==0) {
...
#endif
    {
        ...
        if (frame == 1) {
            ...
            if (last_frame < now) {
                ...
                if (g_cobalt.showfps) LOGD("fps: %.2f, avg: %.2f\n", fps, avg);
#ifdef PANDORA
                if (sock>-1) {
                    char tmp[60];
...
void cobalt_SwapBuffers_currentContext()
{
    if(glxContext)
        cobalt_glXSwapBuffers(glxContext->display, glxContext->drawable);
}

int cobalt_glXGetConfig(Display *display,
                 XVisualInfo *visual,
                 int attribute,
                 int *value) {
    ...
    if(config)
        return cobalt_glXGetFBConfigAttrib(display, *config, attribute, value);
    else
        return get_config_default(display, attribute, value);
}


int cobalt_glXQueryContext( Display *dpy, GLXContext ctx, int attribute, int *value ) {
...
GLXContext cobalt_glXGetCurrentContext() {
...
GLXFBConfig *cobalt_glXChooseFBConfig(Display *display, int screen,
    ...
    *count = 1;
    ...
    if(attrib_list) {
		...
		while(attrib_list[i]!=0) {
			switch(attrib_list[i++]) {
                ...
                case GLX_X_VISUAL_TYPE:
                    tmp = attrib_list[i++];
                    if(!(g_cobalt.usepbuffer || g_cobalt.usefb || g_cobalt.usefbo || g_cobalt.glxnative)) {
    ...
    attr[1] |= (g_cobalt.usepbuffer)?(/*EGL_PBUFFER_BIT|*/EGL_PIXMAP_BIT):EGL_WINDOW_BIT;
    ...
    if (eglInitialized == false) {
        if(!InitEGL((g_cobalt.usefb || g_cobalt.usepbuffer)?g_display:display)) {
            CheckEGLErrors();
    ...
    if(glxconfig==-1) {
        ...
        if((*count==0) && (g_cobalt.usepbuffer)) {
        ...
        if((*count==0) && (!g_cobalt.usepbuffer) && ca && attr[ca+1]) {
                DBG(printf("glXChooseFBConfig found 0 config with an Alpha channel, trying without\n");)
                attr[ca] = 0;
                egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        }
        if((*count==0) && (!g_cobalt.usepbuffer) && ca && attr[ca+1] && (attr[cr]>5 || attr[cg]>5 || attr[cb]>5)) {
...
#ifdef PANDORA
        if((*count==0) && (!g_cobalt.usepbuffer) && (attr[cr]>5 || attr[cg]>5 || attr[cb]>5)) {
...
GLXFBConfig *cobalt_glXGetFBConfigs(Display *display, int screen, int *count) {
    ...
    *count = 1;
    ...
    int tmp;
    attr[cur++] = EGL_SURFACE_TYPE;
    attr[cur++] = (g_cobalt.usepbuffer)?(EGL_PBUFFER_BIT|EGL_PIXMAP_BIT):EGL_WINDOW_BIT;
    attr[cur++] = EGL_RENDERABLE_TYPE;
    attr[cur++] = (hardext.esversion==1)?EGL_OPENGL_ES_BIT:EGL_OPENGL_ES2_BIT;
    attr[cur++] = EGL_NONE; // end list
...
int cobalt_glXGetFBConfigAttrib(Display *display, GLXFBConfig config, int attribute, int *value) {
...
XVisualInfo *cobalt_glXGetVisualFromFBConfig(Display *display, GLXFBConfig config) {
    ...
    XVisualInfo *visuals = XGetVisualInfo(display, VisualDepthMask|VisualClassMask, &xvinfo, &n);
    if (!n) {
        LOGD("Warning, cobalt_glXGetVisualFromFBConfig: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
        return NULL;
    }
    return visuals;
}

GLXContext cobalt_glXCreateNewContext(Display *display, GLXFBConfig config,
    ...
    if(config && (config->drawableType==GLX_PBUFFER_BIT)) {
        ...
        return cobalt_glXCreateContextAttribsARB(display, config, share_list, is_direct, NULL);
        //return glXCreateContext(display, 0, share_list, is_direct);
}
#endif //NOX11

void cobalt_glXSwapInterval(int interval) {
    ...
    if (! g_cobalt.vsync) {
        static int warned = 0;
        if(!warned) {
            LOGD("Enable COBALT_VSYNC=1 if you want to use vsync.\n");
            warned++;
        }
    }
    ...
    if(glxContext) {
        if(g_cobalt.usepbuffer) {
            ...
            if(interval<minswap || interval>maxswap) {
                COBALT_LOGE("Warning, Swap Interval %d is out of possible values %d, %d\n", interval, minswap, maxswap);
            } else
                swapinterval = interval;
        }
    } else {
        DBG(printf("COBALT: glXSwapInterval called before Context is current.\n");)
        swapinterval = interval;
    }
#endif
}
...
#ifndef NOX11
void cobalt_glXSwapIntervalEXT(Display *display, int drawable, int interval) {
    cobalt_glXSwapInterval(interval);
}

// misc stubs
void cobalt_glXCopyContext(Display *display, GLXContext src, GLXContext dst, GLuint mask) {
...
Window cobalt_glXCreateWindow(Display *display, GLXFBConfig config, Window win, int *attrib_list) {
    // should return GLXWindow
    DBG(printf("glXCreateWindow(%p, %p, %p, %p)\n", display, config, (void*)win, attrib_list);)
    return win;
}
void cobalt_glXDestroyWindow(Display *display, void *win) {
    // really wants a GLXWindow
    DBG(printf("glXDestroyWindow(%p, %p)\n", display, win);)
} 

GLXDrawable cobalt_glXGetCurrentDrawable() {
...
Bool cobalt_glXIsDirect(Display * display, GLXContext ctx) {
    DBG(printf("glXIsDirect(%p, %p)\n", display, ctx);)
    return true;
}

void cobalt_glXUseXFont(Font font, int first, int count, int listBase) {
    ...
    cobalt_glGetIntegerv(GL_UNPACK_SWAP_BYTES, &swapbytes);
    cobalt_glGetIntegerv(GL_UNPACK_LSB_FIRST, &lsbfirst);
    cobalt_glGetIntegerv(GL_UNPACK_ROW_LENGTH, &rowlength);
    cobalt_glGetIntegerv(GL_UNPACK_SKIP_ROWS, &skiprows);
    cobalt_glGetIntegerv(GL_UNPACK_SKIP_PIXELS, &skippixels);
    cobalt_glGetIntegerv(GL_UNPACK_ALIGNMENT, &alignment);
	// Set Safe Texture params
	cobalt_glPixelStorei(GL_UNPACK_SWAP_BYTES, GL_FALSE);
    cobalt_glPixelStorei(GL_UNPACK_LSB_FIRST, GL_FALSE);
    ...
    cobalt_glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    ...
    for (i = 0; i < count; i++) {
      ...
      /* glBitmap()' parameters:
       ...
       bm_width = (width + 7) / 8;
       bm_height = height;
       cobalt_glNewList(list, GL_COMPILE);
       if (valid && (bm_width > 0) && (bm_height > 0)) {

          memset(bm, '\0', bm_width * bm_height);
          fill_bitmap(dpy, win, gc, bm_width, bm_height, x, y, c, bm);

          cobalt_glBitmap(width, height, x0, y0, dx, dy, bm);
       }
       else {
          cobalt_glBitmap(0, 0, 0.0, 0.0, dx, dy, NULL);
       }
       cobalt_glEndList();
    }
    ...
    cobalt_glPixelStorei(GL_UNPACK_SWAP_BYTES, swapbytes);
    cobalt_glPixelStorei(GL_UNPACK_LSB_FIRST, lsbfirst);
    cobalt_glPixelStorei(GL_UNPACK_ROW_LENGTH, rowlength);
    cobalt_glPixelStorei(GL_UNPACK_SKIP_ROWS, skiprows);
    cobalt_glPixelStorei(GL_UNPACK_SKIP_PIXELS, skippixels);
    cobalt_glPixelStorei(GL_UNPACK_ALIGNMENT, alignment);
	// All done
}
#endif //NOX11
void cobalt_glXWaitGL() {}
void cobalt_glXWaitX() {}
void cobalt_glXReleaseBuffersMESA() {}

#ifndef NOX11
/* TODO proper implementation */
int cobalt_glXQueryDrawable(Display *dpy, GLXDrawable draw, int attribute, unsigned int *value) {
...
void cobalt_glXDestroyPbuffer(Display * dpy, GLXPbuffer pbuf) {
...
GLXPbuffer cobalt_glXCreatePbuffer(Display * dpy, GLXFBConfig config, const int * attrib_list) {
...
int createPixBuffer(Display * dpy, int bpp, const EGLint * egl_attribs, NativePixmapType nativepixmap, EGLSurface* Surface, EGLContext* Context) {
    ...
    EGLBoolean result;
    if (eglDisplay == NULL || eglDisplay == EGL_NO_DISPLAY) {
        init_display((g_cobalt.usefb || g_cobalt.usepbuffer)?g_display:dpy);
        if (eglDisplay == EGL_NO_DISPLAY) {
            CheckEGLErrors();
            LOGE("Unable to create EGL display.\n");
            return 0;
    ...
    // first time?
    if (eglInitialized == false) {
        result = InitEGL((g_cobalt.usefb || g_cobalt.usepbuffer)?g_display:dpy);
        if (!result) {
            CheckEGLErrors();
            LOGE("Unable to initialize EGL display.\n");
            return 0;
...
GLXPixmap cobalt_glXCreateGLXPixmap(Display *display, XVisualInfo * visual, Pixmap pixmap) {
    ...
    if(g_cobalt.usefb || createPixBuffer(display, depth, NULL, (NativePixmapType)pixmap, &Surface, &Context)==0) {
        // fail, so emulate with a PBuffer
        COBALT_LOGE("Pixmap creation failed, trying PBuffer instead\n");
...
GLXPixmap cobalt_glXCreatePixmap(Display * dpy, GLXFBConfig config, Pixmap pixmap, const int * attrib_list) {
    DBG(printf("glXCreatePixmap(%p, %p, %p, %p)\n", dpy, config, (void*)pixmap, attrib_list);)
    // Check that the config is for PBuffer
    if(!(config->drawableType&GLX_PIXMAP_BIT))
        return 0;
    
    return cobalt_glXCreateGLXPixmap(dpy, NULL, pixmap);
}


void cobalt_glXDestroyGLXPixmap(Display *display, void *pixmap) {
...
void cobalt_glXDestroyPixmap(Display *display, void *pixmap) {
    DBG(printf("glXDestroyPixmap(%p, %p)\n", display, pixmap);)
    cobalt_glXDestroyGLXPixmap(display, pixmap);
}
...
void BlitEmulatedPixmap(int win) {
    ...
    if(reverse) {
        ...
        if(width!=Width || height!=Height /*|| depth!=Depth*/) {
            ...
            DBG(printf("COBALT: Recreate PBuffer %dx%dx%d => %dx%dx%d\n", Width, Height, Depth, width, height, depth);)
...
#endif
    cobalt_glReadPixels(0, 0, Width, Height, (Depth==16)?GL_RGB:GL_BGRA, (Depth==16)?GL_UNSIGNED_SHORT_5_6_5:GL_UNSIGNED_BYTE, (void*)pix);

    actualBlit(reverse, Width, Height, Depth, dpy, drawable, gc, frame, pix, tmp);

}

GLXContext cobalt_glXCreateContextAttribs(Display *dpy, GLXFBConfig config, GLXContext share_context, Bool direct, const int *attrib_list) {
    ...
    if(majver*10+minver != 0) {
        ...
        if(majver*10+minver>g_cobalt.gl) {
            LOGE("Asked for unsupported context version %d.%d (max version is %d.%d)\n", majver, minver, g_cobalt.gl/10, g_cobalt.gl%10);
            return 0;
        }
    ...
    if(mask&GLX_CONTEXT_ES2_PROFILE_BIT_EXT)
        globales2 = 1;
    GLXContext context = cobalt_glXCreateNewContext(dpy, config, GLX_RGBA_TYPE, share_context, direct);
    globales2 = 0;
    return context;
}
#ifndef NO_EGL
...
const char *cobalt_glXQueryExtensionsString(Display *display, int screen) {
    ...
    if(!inited) {
        ...
        if(g_cobalt.es>1 && !g_cobalt.noes2)
...
const char *cobalt_glXQueryServerString(Display *display, int screen, int name) {
    DBG(printf("glXQueryServerString(%p, %d, %d)\n", display, screen, name);)
    switch (name) {
        case GLX_VENDOR: return "ptitSeb";
        case GLX_VERSION: return "1.4 cobalt";
        case GLX_EXTENSIONS: return cobalt_glXQueryExtensionsString(display, screen);
    }
    return 0;    
}

Bool cobalt_glXQueryExtension(Display *display, int *errorBase, int *eventBase) {
...
Bool cobalt_glXQueryVersion(Display *display, int *major, int *minor) {
...
const char *cobalt_glXGetClientString(Display *display, int name) {
    DBG(printf("glXGetClientString(%p, %d)\n", display, name);)
    switch (name) {
        case GLX_VENDOR: return "ptitSeb";
        case GLX_VERSION: return "1.4 cobalt";
        case GLX_EXTENSIONS: return cobalt_glXQueryExtensionsString(display, 0);
    }
    return 0;    
}
