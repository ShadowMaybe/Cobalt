#include "../gl/host.h"
#include "hardext.h"

#include "../gl/debug.h"
#include "../gl/cobalt.h"
#include "../gl/init.h"
#include "../gl/logs.h"
#include "../gl/loader.h"
#ifndef ANDROID
#include "rpi.h"
#endif
#include "glx_gbm.h"

#ifndef EGL_PLATFORM_GBM_KHR
#define EGL_PLATFORM_GBM_KHR                     0x31D7
#endif

static int tested = 0;

hardext_t hardext = {0};

int testGenericShader(struct shader_s* shader_source) {
    // check the current shader is valid for compilation
    
    
    
    
    

    GLuint shad = host_functions.glCreateShader(shader_source->type);
    host_functions.glShaderSource(shad, 1, (const GLchar *const *)(&shader_source->converted), NULL);
    host_functions.glCompileShader(shad);
    GLint compiled;
    host_functions.glGetShaderiv(shad, GL_COMPILE_STATUS, &compiled);
    /*
    if(!compiled) {
        
        char buff[500];
        host_functions.glGetShaderInfoLog(shad, 500, NULL, buff);
        printf("COBALT: \"%s\" failed, message:\n%s\n", version, buff);
    }
    */
    host_functions.glDeleteShader(shad);

    return compiled;
}

static int testGLSL(const char* version, int uniformLoc) {
    // check if glsl 120 shaders are supported... by compiling one !
    
    
    
    
    
    

    GLuint shad = host_functions.glCreateShader(GL_VERTEX_SHADER);
    const char* shadTest[4] = {
        version,
        "\n"
        "layout(location = 0) in vec4 vecPos;\n",
        uniformLoc?"layout(location = 0) uniform mat4 matMVP;\n":"uniform mat4 matMVP;\n",
        "void main() {\n"
        " gl_Position = matMVP * vecPos;\n"
        "}\n"
    };
    host_functions.glShaderSource(shad, 4, shadTest, NULL);
    host_functions.glCompileShader(shad);
    GLint compiled;
    host_functions.glGetShaderiv(shad, GL_COMPILE_STATUS, &compiled);
    /*
    if(!compiled) {
        
        char buff[500];
        host_functions.glGetShaderInfoLog(shad, 500, NULL, buff);
        printf("COBALT: \"%s\" failed, message:\n%s\n", version, buff);
    }
    */
    host_functions.glDeleteShader(shad);
    host_functions.glGetError();	// reset GL Error

    return compiled;
}

static int testTextureCubeLod() {
    
    
    
    
    
    

    GLuint shad = host_functions.glCreateShader(GL_FRAGMENT_SHADER);
    const char* shadTest[3] = {
        "#version 100",
        "\n"
        "#extension GL_EXT_shader_texture_lod : enable\n"
        "uniform samplerCube samCube;\n"
        "varying mediump vec3 coordCube;\n",
        "void main() {\n"
        " gl_FragColor = textureCubeLod(samCube, coordCube, 0.0);\n"
        "}\n"
    };
    host_functions.glShaderSource(shad, 3, shadTest, NULL);
    host_functions.glCompileShader(shad);
    GLint compiled;
    host_functions.glGetShaderiv(shad, GL_COMPILE_STATUS, &compiled);
    host_functions.glDeleteShader(shad);
    host_functions.glGetError(); // reset GL Error

    return compiled;
}

EXPORT
void GetHardwareExtensions(int notest)
{
    if(tested) return;
    // put some default values
    hardext.maxtex = 2;
    hardext.maxsize = 2048;
    hardext.maxlights = 8;
    hardext.maxplanes = 6;
    hardext.maxdrawbuffers = 1;

    hardext.esversion = g_cobalt.es;
    if(notest) 
    {
#ifndef AMIGAOS4
        COBALT_LOGD("Hardware test disabled, nothing activated...\n");
#endif
        if(hardext.esversion==2) {
            hardext.maxteximage = 4;
            hardext.maxvarying = 8;
            hardext.maxtex = 8;
            hardext.maxvattrib = 16;
#ifdef AMIGAOS4
            hardext.npot = 3;
#else
            hardext.npot = 1;
#endif
            hardext.fbo = 1; 
            hardext.blendcolor = 1;
            hardext.blendsub = 1;
            hardext.blendfunc = 1;
            hardext.blendeq = 1;
            hardext.mirrored = 1;
            hardext.pointsprite = 1;
            hardext.pointsize = 1;
            hardext.cubemap = 1;
            hardext.maxdrawbuffers = 1;
#ifdef AMIGAOS4
            hardext.glsl300es = 1;
#endif
        }
        return;
    }
#if defined(BCMHOST) && !defined(ANDROID)
    rpi_init();
#endif
#ifdef NOEGL
    COBALT_LOGD("Hardware test on current Context...\n");
#else
    // used EGL & GLES functions
    LOAD_EGL(eglBindAPI);
    LOAD_EGL(eglInitialize);
    LOAD_EGL(eglGetDisplay);
    LOAD_EGL(eglCreatePbufferSurface);
    LOAD_EGL(eglDestroySurface);
    LOAD_EGL(eglDestroyContext);
    LOAD_EGL(eglMakeCurrent);
    LOAD_EGL(eglChooseConfig);
    LOAD_EGL(eglCreateContext);
    LOAD_EGL(eglQueryString);
    LOAD_EGL(eglTerminate);

    EGLDisplay eglDisplay;
    EGLSurface eglSurface;
    EGLContext eglContext;

    COBALT_LOGD("Using GLES %s backend\n", (hardext.esversion==1)?"1.1":"2.0");

    // Create a PBuffer first...
    EGLint egl_context_attrib_es2[] = {
        EGL_CONTEXT_CLIENT_VERSION, 2,
        EGL_NONE
    };

    EGLint egl_context_attrib[] = {
        EGL_NONE
    };

    //let's create a PBuffer attributes
    EGLint egl_attribs[10];	// should be enough
    int i = 0;
    egl_attribs[i++] = EGL_WIDTH;
    egl_attribs[i++] = 32;
    egl_attribs[i++] = EGL_HEIGHT;
    egl_attribs[i++] = 32;
    egl_attribs[i++] = EGL_NONE;

    EGLint configAttribs[] = {
#ifdef PANDORA
    // on the Pandora, there don't seem to exist a 8888 PBuffer config for GLES2.
        EGL_RED_SIZE, (hardext.esversion==1)?8:5,
        EGL_GREEN_SIZE, (hardext.esversion==1)?8:6,
        EGL_BLUE_SIZE, (hardext.esversion==1)?8:5,
        EGL_ALPHA_SIZE, (hardext.esversion==1)?8:0,
#else
        EGL_RED_SIZE, 8,
        EGL_GREEN_SIZE, 8,
        EGL_BLUE_SIZE, 8,
        EGL_ALPHA_SIZE, 8,
#endif
        EGL_SURFACE_TYPE, EGL_PBUFFER_BIT,
        EGL_RENDERABLE_TYPE, (hardext.esversion==1)?EGL_OPENGL_ES_BIT:EGL_OPENGL_ES2_BIT,
        EGL_NONE
    };

    int configsFound;
    static EGLConfig pbufConfigs[1];

#ifndef NO_GBM
    if (g_cobalt.usegbm) {
        LoadGBMFunctions();
        eglDisplay = OpenGBMDisplay(EGL_DEFAULT_DISPLAY);
    }
    else
#endif
        eglDisplay = egl_eglGetDisplay(EGL_DEFAULT_DISPLAY);

    egl_eglBindAPI(EGL_OPENGL_ES_API);
    if (egl_eglInitialize(eglDisplay, NULL, NULL) != EGL_TRUE) {
        LOGE("Error while gathering supported extension (eglInitialize: %s), default to none\n", PrintEGLError(0));
        egl_eglTerminate(eglDisplay);
        return;
    }

    egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);
#ifndef NO_GBM
    const char* eglExts = egl_eglQueryString(EGL_NO_DISPLAY, EGL_EXTENSIONS);
    if(eglExts && strstr(eglExts, "EGL_KHR_platform_gbm ")) {
        COBALT_LOGD("GBM Surfaces supported%s\n", g_cobalt.usegbm?" and used":"");
        hardext.gbm = 1;
    }
#endif
#ifndef PANDORA
    if(!configsFound) {
        // try without alpha channel
        configAttribs[4*2-1] = 0;
        egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);
        if(configsFound) {
            COBALT_LOGD("No alpha channel in PBuffer context, disabling EGL Alpha channel...\n");
            hardext.eglnoalpha = 1;
        }
    }
#endif
    if(!configsFound) {
        COBALT_LOGE("Error while gathering supported extension (eglChooseConfig: %s), default to none\n", PrintEGLError(0));
        egl_eglTerminate(eglDisplay);
        return;
    }
    eglContext = egl_eglCreateContext(eglDisplay, pbufConfigs[0], EGL_NO_CONTEXT, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
    if(!eglContext) {
        COBALT_LOGE("Error while gathering supported extension (eglCreateContext: %s), default to none\n", PrintEGLError(0));
        return;
    }
    eglSurface = egl_eglCreatePbufferSurface(eglDisplay, pbufConfigs[0], egl_attribs);
    if(!eglSurface) {
        COBALT_LOGE("Error while gathering supported extension (eglCreatePBufferSurface: %s), default to none\n", PrintEGLError(0));
        egl_eglDestroyContext(eglDisplay, eglContext);
        egl_eglTerminate(eglDisplay);
        return;
    }
    egl_eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext);
#endif
    tested = 1;
    
    
    
    // Now get extensions
    const char *Exts = (const char *) host_functions.glGetString(GL_EXTENSIONS);
    // Parse them!
    #define S(A, B, C) if(strstr(Exts, A)) { hardext.B = 1; COBALT_LOGD("Extension %s detected%s",A, C?" and used\n":"\n"); } 
    if(hardext.esversion>1) hardext.npot = 1;
    if(strstr(Exts, "GL_APPLE_texture_2D_limited_npot ")) hardext.npot = 1;
    if(strstr(Exts, "GL_IMG_texture_npot ")) hardext.npot = 1; // it should enable mipmap (so hardext.npot=2), but mipmap (so level > 0) needs to be POT-sized?!!
    if(strstr(Exts, "GL_ARB_texture_non_power_of_two ") || strstr(Exts, "GL_OES_texture_npot ")) hardext.npot = 3;
    if(hardext.npot>0) {
        COBALT_LOGD("Hardware %s NPOT detected and used\n", hardext.npot==3?"Full":(hardext.npot==2?"Limited+Mipmap":"Limited"));
    }
    S("GL_EXT_blend_minmax ", blendminmax, 1);
    S("GL_EXT_draw_buffers ", drawbuffersext, 1);
    // Even if drawbuffersext is 0, it would be fine as ESSL version detection (and thus, ES3 detection) comes later
    // and will set hardext.drawbuffers to the correct value
    hardext.drawbuffers = hardext.drawbuffersext; 
    
    /*if(hardext.blendcolor==0) {
        // try by just loading the function
        
        if(host_functions.glBlendColor != NULL) {
            hardext.blendcolor = 1;
	        COBALT_LOGD("Extension glBlendColor found and used\n");
	    }
    }*/ // I don't think this is correct
    if(hardext.esversion<2) {
        S("GL_OES_framebuffer_object ", fbo, 1);
        S("GL_OES_point_sprite ", pointsprite, 1); 
        S("GL_OES_point_size_array ", pointsize, 0);
        S("GL_OES_texture_cube_map ", cubemap, 1);
        S("GL_EXT_blend_color ", blendcolor, 1);
        S("GL_OES_blend_subtract ", blendsub, 1);
        S("GL_OES_blend_func_separate ", blendfunc, 1);
        S("GL_OES_blend_equation_separate ", blendeq, 1);
        S("GL_OES_texture_mirrored_repeat ", mirrored, 1);  
    } else {
        hardext.fbo = 1; 
        COBALT_LOGD("FBO are in core, and so used\n");
        hardext.pointsprite = 1;
        COBALT_LOGD("PointSprite are in core, and so used\n");
        hardext.pointsize = 1;
        COBALT_LOGD("CubeMap are in core, and so used\n");
        hardext.cubemap = 1;
        COBALT_LOGD("BlendColor is in core, and so used\n");
        hardext.blendcolor = 1;
        COBALT_LOGD("Blend Subtract is in core, and so used\n");
        hardext.blendsub = 1;
        COBALT_LOGD("Blend Function and Equation Separation is in core, and so used\n");
        hardext.blendfunc = 1;
        hardext.blendeq = 1;
        COBALT_LOGD("Texture Mirrored Repeat is in core, and so used\n");
        hardext.mirrored = 1;
    }
    S("GL_OES_mapbuffer ", mapbuffer, 0);
    S("GL_OES_element_index_uint ", elementuint, 1);
    S("GL_OES_packed_depth_stencil ", depthstencil, 1);
    S("GL_OES_depth24 ", depth24, 1);
    S("GL_OES_rgb8_rgba8 ", rgba8, 1);
    S("GL_EXT_multi_draw_arrays ", multidraw, 0);
    if(!g_cobalt.nobgra) {
        S("GL_EXT_texture_format_BGRA8888 ", bgra8888, 1);
    }
    if(!g_cobalt.nodepthtex) {
        S("GL_OES_depth_texture ", depthtex, 1);
        S("GL_OES_texture_stencil8 ", stenciltex, 1);
    }
    S("GL_OES_draw_texture ", drawtex, 1);
    S("GL_EXT_texture_rg ", rgtex, 1);
    if(g_cobalt.floattex) {
        S("GL_OES_texture_float ", floattex, 1);
        S("GL_OES_texture_half_float ", halffloattex, 1);
        S("GL_EXT_color_buffer_float ", floatfbo, 1);
        S("GL_EXT_color_buffer_half_float ", halffloatfbo, 1);
    }
    S("GL_AOS4_texture_format_RGB332", rgb332, 0);
    S("GL_AOS4_texture_format_RGB332REV", rgb332rev, 0);
    S("GL_AOS4_texture_format_RGBA1555REV", rgba1555rev, 1);
    S("GL_AOS4_texture_format_RGBA8888", rgba8888, 1);
    S("GL_AOS4_texture_format_RGBA8888REV", rgba8888rev, 1);

    if (hardext.esversion>1) {
        if(!g_cobalt.nohighp) {
            S("GL_OES_fragment_precision_high ", highp, 1);
            if(!hardext.highp) {
                // check if highp is supported anyway
                
                if(host_functions.glGetShaderPrecisionFormat) {
                    GLint range[2] = {0};
                    GLint precision=0;
                    host_functions.glGetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_HIGH_FLOAT, range, &precision);
                    if(!(range[0]==0 && range[1]==0 && precision==0)) {
                        hardext.highp = 2;  // no need to declare #entension here
                        COBALT_LOGD("high precision float in fragment shader available and used\n");
                    }
                }
            }
        }
        if(!g_cobalt.noshaderlod) 
            S("GL_EXT_shader_texture_lod", shaderlod, 1);
        if(hardext.shaderlod) {
            // test is textureCubeLod need EXT or not (seems to be a bug in some PVR driver)
            if(testTextureCubeLod()) {
                hardext.cubelod = 1;
                COBALT_LOGD("textureCubeLod in fragment doesn't need trailing EXT\n");
            }
        }
        S("GL_EXT_frag_depth ", fragdepth, 1);
        host_functions.glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &hardext.maxvattrib);
        COBALT_LOGD("Max vertex attrib: %d\n", hardext.maxvattrib);
        S("GL_OES_standard_derivatives ", derivatives, 1);
        S("GL_ARM_shader_framebuffer_fetch", shader_fbfetch, 1);
        S("GL_OES_get_program ", prgbinary, 1);
        if(!hardext.prgbinary) {
            S("GL_OES_get_program_binary ", prgbinary, 1);
        }
        if(hardext.prgbinary) {
            host_functions.glGetIntegerv(GL_NUM_PROGRAM_BINARY_FORMATS_OES, &hardext.prgbin_n);
            COBALT_LOGD("Number of supported Program Binary Format: %d\n", hardext.prgbin_n);
        }
    }
    // Now get some max stuffs
    host_functions.glGetIntegerv(GL_MAX_TEXTURE_SIZE, &hardext.maxsize);
    COBALT_LOGD("Max texture size: %d\n", hardext.maxsize);
    host_functions.glGetIntegerv((hardext.esversion==1)?GL_MAX_TEXTURE_UNITS:GL_MAX_TEXTURE_IMAGE_UNITS, &hardext.maxtex);
    if (hardext.esversion==1) {
        host_functions.glGetIntegerv(GL_MAX_LIGHTS, &hardext.maxlights);
        host_functions.glGetIntegerv(GL_MAX_CLIP_PLANES, &hardext.maxplanes);
        hardext.maxteximage=hardext.maxtex;
    } else {
        // simulated stuff using the FPE
        hardext.maxlights = 8;
        hardext.maxplanes = 6;
        host_functions.glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &hardext.maxteximage);
        host_functions.glGetIntegerv(GL_MAX_VARYING_VECTORS, &hardext.maxvarying);
        COBALT_LOGD("Max Varying Vector: %d\n", hardext.maxvarying);
        if(hardext.maxvattrib<16 && hardext.maxtex>4)
            hardext.maxtex = 4; // with less then 16 vertexattrib, more then 4 textures seems unreasonnable
    }
    int hardmaxtex = hardext.maxtex;
    if(hardext.maxtex>MAX_TEX) hardext.maxtex=MAX_TEX;      // caping, as there are some fixed-sized array...
    if(hardext.maxteximage>MAX_TEX) hardext.maxteximage=MAX_TEX;
    if(hardext.maxlights>MAX_LIGHT) hardext.maxlights=MAX_LIGHT;                // caping lights too
    if(hardext.maxplanes>MAX_CLIP_PLANES) hardext.maxplanes=MAX_CLIP_PLANES;    // caping planes, even 6 should be the max supported anyway
    COBALT_LOGD("Texture Units: %d/%d (hardware: %d), Max lights: %d, Max planes: %d\n", hardext.maxtex, hardext.maxteximage, hardmaxtex, hardext.maxlights, hardext.maxplanes);
    S("GL_EXT_texture_filter_anisotropic ", aniso, 1);
    if(hardext.aniso) {
        host_functions.glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY, &hardext.aniso);
        if(host_functions.glGetError()!=GL_NO_ERROR)
            hardext.aniso = 0;
        if(hardext.aniso)
            COBALT_LOGD("Max Anisotropic filtering: %d\n", hardext.aniso);
    }
    
    // get GLES driver signatures...
    const char *vendor = (const char *) host_functions.glGetString(GL_VENDOR);
    COBALT_LOGD("Hardware vendor is %s\n", vendor);
    if(strstr(vendor, "ARM"))
        hardext.vendor = VEND_ARM;
    else if(strstr(vendor, "Imagination Technologies"))
        hardext.vendor = VEND_IMGTEC;
    if(hardext.esversion>1) {
        /*
        if(testGLSL("#version 120", 1))
            hardext.glsl120 = 1;
        */

        // Test ogl version, only supported on ogl(es) 3+
        GLint major_version = 0;
        host_functions.glGetIntegerv(GL_MAJOR_VERSION, &major_version);
        host_functions.glGetError(); // Delete the error

        if(major_version >= 3) {
            hardext.glsl300es = 1;

            GLint minor_version = 0;
            host_functions.glGetIntegerv(GL_MINOR_VERSION, &minor_version);

            if(minor_version >= 1){
                hardext.glsl310es = 1;
            }
            if(minor_version >= 2){
                hardext.glsl320es = 1;
            }
        }
    }
    if(hardext.glsl120) {
        COBALT_LOGD("GLSL 120 supported and used\n");
    }



    if(hardext.glsl300es) {
        COBALT_LOGD("GLSL 300 es supported%s\n", (hardext.glsl120||hardext.glsl300es)?"":" and used");
	    hardext.drawbuffers = 1;
    }
    if(hardext.glsl310es) {
        COBALT_LOGD("GLSL 310 es supported%s\n", hardext.glsl120?"":" and used");
	    hardext.drawbuffers = 1;
    }
    // VGPU SPECIFIC
    if(hardext.glsl320es) {
        COBALT_LOGD("GLSL 320 es supported%s\n", hardext.glsl320es?"":" and used");
	    hardext.drawbuffers = 1;
    }

    if(!hardext.glsl300es){
        // We can't use the vgpu forward conversion
        g_cobalt.vgpu_backport = 1;
    }
	
    if(hardext.drawbuffers) {
        host_functions.glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS_EXT,&hardext.maxcolorattach);
        host_functions.glGetIntegerv(GL_MAX_DRAW_BUFFERS_ARB, &hardext.maxdrawbuffers);
    }
    if(hardext.maxcolorattach<1)
        hardext.maxcolorattach = 1;
    if(hardext.maxcolorattach>MAX_DRAW_BUFFERS)
        hardext.maxcolorattach=MAX_DRAW_BUFFERS;
    if(hardext.maxdrawbuffers<1)
        hardext.maxdrawbuffers = 1;
    if(hardext.maxdrawbuffers>MAX_DRAW_BUFFERS)
        hardext.maxdrawbuffers=MAX_DRAW_BUFFERS;
    COBALT_LOGD("Max Color Attachments: %d / Draw buffers: %d\n", hardext.maxdrawbuffers, hardext.maxcolorattach);
#ifndef NOEGL
    if(strstr(egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_colorspace")) {
        COBALT_LOGD("sRGB surface supported\n");
        hardext.srgb = 1;
    }
    if(strstr(egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_image_pixmap")) {
        COBALT_LOGD("EGLImage from Pixmap supported\n");
        hardext.khr_pixmap = 1;
    }
    if(strstr(egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_texture_2D_image")) {
        COBALT_LOGD("EGLImage to Texture2D supported\n");
        hardext.khr_texture_2d = 1;
    }
    if(strstr(egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_renderbuffer_image")) {
        COBALT_LOGD("EGLImage to RenderBuffer supported\n");
        hardext.khr_renderbuffer = 1;
    }

    // End, cleanup
    egl_eglMakeCurrent(eglDisplay, 0, 0, EGL_NO_CONTEXT);
    egl_eglDestroySurface(eglDisplay, eglSurface);
    egl_eglDestroyContext(eglDisplay, eglContext);

    egl_eglTerminate(eglDisplay);
#endif

// After extensions are gathered, we need to load the extension functions
init_gl_functions();
}
