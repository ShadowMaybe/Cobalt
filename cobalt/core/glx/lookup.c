...
void *cobalt_glXGetProcAddress(const char *name) __attribute__((visibility("default")));
void *cobalt_glXGetProcAddress(const char *name) {
...
#endif
    MAP("glXSwapIntervalMESA", cobalt_glXSwapInterval);
    MAP("glXSwapIntervalSGI", cobalt_glXSwapInterval);
#if !defined(NOX11) || defined(GLX_STUBS)
    _EX(glXUseXFont);
    _EX(glXWaitGL);
    _EX(glXWaitX);
    ...
    MAP("glXChooseFBConfigSGIX", cobalt_glXChooseFBConfig);
    ...
    return cobalt_GetProcAddress(name);
}
#ifdef AMIGAOS4
