...
void rpi_init() {
    ...
    if(!bcm_host_init || !bcm_host_deinit) {
        printf("COBALT: Warning, bcm_host function missing (init=%p, deinit=%p)\n", bcm_host_init, bcm_host_deinit);
        return;
    }
    bcm_host_init();

    #define GO(A) A=dlsym(bcm_host, #A); if(A==NULL) A=dlsym(vcos, #A); if(A==NULL) printf("COBALT: Warning, " #A " is null")
