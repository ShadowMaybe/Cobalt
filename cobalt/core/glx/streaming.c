...
int alloc_buff(int buff, int width, int height) {
	...
	if (ioctl(bc_cat[buff], BCIOREQ_BUFFERS, &buf_param) != 0) {
		printf("COBALT: BCIOREQ_BUFFERS failed\n");
		return 0;
	}
	if (ioctl(bc_cat[buff], BCIOGET_BUFFERCOUNT, &ioctl_var) != 0) {
		printf("COBALT: BCIOREQ_BUFFERCOUNT failed\n");
		return 0;
	}
	if (ioctl_var.output == 0) {
		printf("COBALT: Streaming, no texture buffer available\n");
		return 0;
	}
	const char *bcdev = glGetTexDeviceIMG(buff);
	if (!bcdev) {
		printf("COBALT: problem with getting the GL_IMG_texture_stream device\n");
		return 0;
	} else {
		bcdev_w = width;
		bcdev_h = height;
		...
		printf("COBALT: Streaming device = %s num: %d, width: %d, height: %d, format: 0x%x\n",
			bcdev, bcdev_n, bcdev_w, bcdev_h, bcdev_fmt);
		if (bcdev_w!=width) {
			printf("COBALT: Streaming not activate, buffer width != asked width\n");
			return 0;
		}
	}
/*	
	...
	if (ioctl(bc_cat[buff], BCIOGET_BUFFERPHYADDR, &ioctl_var) != 0) {
		printf("COBALT: BCIOGET_BUFFERADDR failed\n");
		return 0;
	} else {
...
//printf("ApplyFilterID(%i, 0x%04X, 0x%04X)\n", ID, min_filter, mag_filter);
	...
	if (!stream_cache[ID].active)
		return;
    cobalt_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MIN_FILTER, min_filter);
    cobalt_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MAG_FILTER, mag_filter);
}
