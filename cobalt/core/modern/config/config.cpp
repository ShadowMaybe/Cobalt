...
char* DEFAULT_COBALT_DIRECTORY_PATH = "/sdcard/MG";
...
int check_path() {
    if (!mg_directory_path) {
        char* var = getenv("COBALT_DIR_PATH");
        is_custom_mg_dir = var ? true : false;
        mg_directory_path = var ? strdup(var) : DEFAULT_COBALT_DIRECTORY_PATH;
    }
...
int config_refresh() {
    LOG_D("COBALT_DIRECTORY_PATH=%s", mg_directory_path)
