...
void init_settings() {
    ...
    int isInPluginApp = 0;
    GetEnvVarInt("COBALT_PLUGIN_STATUS", &isInPluginApp, 0);
    int fclVersion = 0;
    ...
    LOG_V("COBALT_DIR_PATH = %s", mg_directory_path ? mg_directory_path : "(default)")
    ...
    if (global_settings.angle == AngleMode::Enabled) {
        // setenv("COBALT_GLES", "libGLESv2_angle.so", 1);
        // setenv("COBALT_EGL", "libEGL_angle.so", 1);
    }
    ...
    LOG_V("[Cobalt] Setting: enableAngle                 = %s",
          global_settings.angle == AngleMode::Enabled ? "true" : "false")
    LOG_V("[Cobalt] Setting: ignoreError                 = %i", static_cast<int>(global_settings.ignore_error))
    LOG_V("[Cobalt] Setting: enableExtComputeShader      = %s",
          global_settings.ext_compute_shader ? "true" : "false")
    LOG_V("[Cobalt] Setting: enableExtTimerQuery         = %s", global_settings.ext_timer_query ? "true" : "false")
    LOG_V("[Cobalt] Setting: enableExtDirectStateAccess  = %s",
          global_settings.ext_direct_state_access ? "true" : "false")
    LOG_V("[Cobalt] Setting: maxGlslCacheSize            = %i",
          static_cast<int>(global_settings.max_glsl_cache_size / 1024 / 1024))
    LOG_V("[Cobalt] Setting: angleDepthClearFixMode      = %i",
          static_cast<int>(global_settings.angle_depth_clear_fix_mode))
    LOG_V("[Cobalt] Setting: bufferCoherentAsFlush       = %i",
          static_cast<int>(global_settings.buffer_coherent_as_flush))
    if (global_settings.custom_gl_version.isEmpty()) {
        LOG_V("[Cobalt] Setting: customGLVersion             = (default)");
    } else {
        LOG_V("[Cobalt] Setting: customGLVersion             = %s",
              global_settings.custom_gl_version.toString().c_str());
    }
    LOG_V("[Cobalt] Setting: fsr1Setting                 = %i", static_cast<int>(global_settings.fsr1_setting))
    LOG_V("[Cobalt] Setting: hideMGEnvLevel              = %i",
...
void init_settings_post() {
    ...
    for (int i = 0; i < MD_ENTRY_COUNT; ++i) {
        ...
        LOG_V("[Cobalt] %-34s = %s", k_md_entries[i].order_key, order_str.c_str())
    }
}
