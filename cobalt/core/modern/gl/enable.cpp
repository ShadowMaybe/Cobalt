...
bool is_clip_distance(GLenum cap, GLuint* slot) {
    ...
    if (n >= COBALT_MAX_CLIP_DISTANCES) return false;
    *slot = n;
    return true;
}
...
void mg_enable_reset(mg_enable_state_t* state) {
    ...
    for (int i = 0; i < COBALT_MAX_DRAW_BUFFERS; ++i) state->blend_indexed[i] = GL_FALSE;
    for (int i = 0; i < COBALT_MAX_VIEWPORTS; ++i) state->scissor_indexed[i] = GL_FALSE;
...
GLboolean mg_enable_get(GLenum cap, GLuint index) {
    ...
    if (d->cap == GL_BLEND && index < COBALT_MAX_DRAW_BUFFERS) return st->blend_indexed[index];
    if (d->cap == GL_SCISSOR_TEST && index < COBALT_MAX_VIEWPORTS) return st->scissor_indexed[index];
...
bool mg_enable_query_int(GLenum pname, GLint* out) {
    ...
    case GL_MAX_DRAW_BUFFERS: {
        ...
        if (n <= 0) n = 1;
        *out = n > COBALT_MAX_DRAW_BUFFERS ? COBALT_MAX_DRAW_BUFFERS : n;
        return true;
    }
    default:
        return false;
...
static void mg_set_enabled(GLenum cap, GLuint index, bool indexed, GLboolean value) {
    ...
    if (indexed) {
        ...
        if (d->cap == GL_BLEND) {
            if (index >= COBALT_MAX_DRAW_BUFFERS) {
    ...
    if (already_set && d->cap == GL_BLEND) {
        for (int i = 0; i < COBALT_MAX_DRAW_BUFFERS; ++i) {
    ...
    if (d->cap == GL_BLEND) {
        for (int i = 0; i < COBALT_MAX_DRAW_BUFFERS; ++i) st->blend_indexed[i] = value;
    }
    if (d->cap == GL_SCISSOR_TEST) {
        for (int i = 0; i < COBALT_MAX_VIEWPORTS; ++i) st->scissor_indexed[i] = value;
    }
