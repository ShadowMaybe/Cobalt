...
void mg_set_gl_error(GLenum error) {
    ...
    LOG_D("Cobalt raised %s", glEnumToString(error))
}
...
void InitGLESBaseExtensions() {
    ...
    if (global_settings.hide_mg_env_level == HideMGEnvLevel::Disabled) {
        extensions.push_back("GL_COBALT_cobalt");
        extensions.push_back("GL_COBALT_backend_string_getter_access");
        extensions.push_back("GL_COBALT_settings_string_dump");
    }
        ...
        if (versionString.empty()) {
            ...
            if (global_settings.hide_mg_env_level == HideMGEnvLevel::Disabled) {
                if (GLVersion.toInt(2) == DEFAULT_GL_VERSION) {
                    versionString += " Cobalt ";
                } else {
                    Version defaultVersion = Version(DEFAULT_GL_VERSION);
                    versionString += " §4§l(" + defaultVersion.toString() + ") Cobalt§r ";
                }
    ...
    case GL_SHADING_LANGUAGE_VERSION: {
        ...
        if (shadingLangString.empty()) {
            ...
            if (global_settings.hide_mg_env_level >= HideMGEnvLevel::Level1) {
                ...
                shadingLangString = baseVer + " Cobalt with glslang and SPIRV-Cross";
            }
        }
