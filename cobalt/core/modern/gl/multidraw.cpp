...
#define COBALT_REBASE_LOOP(SRCTYPE)                                                                                        \
    do {                                                                                                               \
    ...
    switch (type) {
    case GL_UNSIGNED_INT:
        COBALT_REBASE_LOOP(GLuint);
        break;
    case GL_UNSIGNED_SHORT:
        COBALT_REBASE_LOOP(GLushort);
        break;
    case GL_UNSIGNED_BYTE:
        COBALT_REBASE_LOOP(GLubyte);
        break;
    default:
        break;
    }

#undef COBALT_REBASE_LOOP
}
...
// dlsym(RTLD_DEFAULT, ...) and would hand back Cobalt' own exported
...
bool mg_multi_draw_arrays_ext_available() {
    ...
    if (!resolved) {
        ...
        // through it would find Cobalt' own alias and recurse. The only thing
