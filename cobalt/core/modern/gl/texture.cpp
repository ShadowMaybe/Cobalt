...
static const int COBALT_TEXTURE_BUFFER_EMULATION_UNIT = 15;
...
void glBindTexture(GLenum target, GLuint texture) {
    ...
    const int driver_unit = emulated_buffer_texture ? COBALT_TEXTURE_BUFFER_EMULATION_UNIT : DriverActiveTextureUnit;
    ...
    if (!redundant) {
        if (emulated_buffer_texture) {
            GLES.glActiveTexture(GL_TEXTURE0 + COBALT_TEXTURE_BUFFER_EMULATION_UNIT);
