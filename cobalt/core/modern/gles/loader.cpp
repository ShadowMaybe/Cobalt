...
//
// COBALT_ANGLE_DIR has three states, and the empty one is not the same as absent:
//
...
static const char* angle_override(const char* name, std::string& storage) {
    const char* dir = getenv("COBALT_ANGLE_DIR");
    if (dir == nullptr) {
        storage = name;
...
void* open_lib(const char** names, const char* override, bool* used_override) {
    ...
    if (override) {
        if ((lib = dlopen(override, flags))) {
            ...
            LOG_D("COBALT:loaded: %s\n", path_name)
            if (used_override) *used_override = true;
            return lib;
        } else {
            LOG_E("COBALT_GLES override failed: %s\n", dlerror())
        }
    }
...
void init_target_gles() {
    ...
    LOG_D("Initializing %s @ hardware", COBALT_RENDERER_NAME)
    set_hardware();
