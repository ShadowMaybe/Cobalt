...
#define DBG(a)
    ...
    GLenum COBALT_API cobalt_glGetError(void) {
        DBG(COBALT_LOGD("glGetError(), noerror=%d, type_error=%d shim_error=%s\n", g_cobalt.noerror,
                      glstate->type_error, PrintEnum(glstate->shim_error));)
        if (g_cobalt.noerror) return GL_NO_ERROR;
    ...
    void COBALT_API cobalt_glGetPointerv(GLenum pname, GLvoid** params) {
        DBG(COBALT_LOGD("glGetPointerv(%s, %p)\n", PrintEnum(pname), params);)
        noerrorShim();
        switch (pname) {
    ...
    void BuildExtensionsList() {
        if (!glstate->extensions) {
            ...
            if (!g_cobalt.notexrect) strcat(extensions, "GL_ARB_texture_rectangle ");
            if (g_cobalt.vabgra) strcat(extensions, "GL_ARB_vertex_array_bgra ");
            if (g_cobalt.npot >= 1) strcat(extensions, "GL_APPLE_texture_2D_limited_npot ");
            if (g_cobalt.npot >= 2) strcat(extensions, "GL_ARB_texture_non_power_of_two ");
            ...
            if (hardext.floattex || (g_cobalt.floattex == 2)) {
                strcat(extensions, "GL_EXT_texture_float ");
                strcat(extensions, "GL_ARB_texture_float ");
            }
            if (hardext.halffloattex || (g_cobalt.floattex == 2)) {
                strcat(extensions, "GL_EXT_texture_half_float ");
            }
            if (hardext.floatfbo || (g_cobalt.floattex == 2)) {
                strcat(extensions, "GL_EXT_color_buffer_float ");
            }
            if (hardext.halffloatfbo || (g_cobalt.floattex == 2)) {
    ...
    void set_es_version() {
        ...
        if (sscanf(ESVersion, "OpenGL ES %d.%d", &major, &minor) == 2) {
            g_cobalt.esversion = major * 100 + minor * 10;
        } else {
            g_cobalt.esversion = g_cobalt.es * 100;
        }
        init_internal_glDrawElementsBaseVertex();
        COBALT_LOGD("OpenGL ES Version: %s (%d)", ESVersion, g_cobalt.esversion);
        if (g_cobalt.esversion < 300) {
            COBALT_LOGD("OpenGL ES version is lower than 3.0! This version is not supported completely!");
            COBALT_LOGD("FBO and texture compression may be completely broken!");
            COBALT_LOGD("Disabled glslang and SPIRV-Cross, using vgpu shaderconv to convert all shaders!");
        } else if (g_cobalt.esversion < 310) {
            COBALT_LOGD("OpenGL ES version is lower than 3.1! Rendering may be broken!");
        } else if (g_cobalt.esversion < 320) {
            COBALT_LOGD("OpenGL ES version is lower than 3.2! Some features may not work as expected!");
        }
    }
    ...
    const GLubyte* COBALT_API cobalt_glGetString(GLenum name) {
        DBG(COBALT_LOGD("glGetString(%s)\n", PrintEnum(name));)
        if (!g_cobalt.esversion) set_es_version();
        errorShim(GL_NO_ERROR);
        switch (name) {
        case GL_VERSION:
            // return (GLubyte *)g_cobalt.version;
            if (versionString.empty()) {
                int use_mc_color = ReturnEnvVarInt("COBALT_USE_MC_COLOR");
                std::ostringstream version_builder;
                version_builder << (g_cobalt.gl / 10) << "." << (g_cobalt.gl % 10);
                ...
                if (use_mc_color == 1) {
                    ...
                    version_builder << " Cobalt Wrapper " << VERSION_TYPE;
                }
            ...
            DBG(COBALT_LOGD("glGetString(GL_VERSION) -> %s", versionString.c_str()))
        ...
        case GL_SHADING_LANGUAGE_VERSION:
            return (GLubyte*)"4.50 Cobalt Wrapper with glslang and SPIRV-Cross";
...
#define TOP(A) (glstate->A->stack + (glstate->A->top * 16))
    ...
    int cobalt_commonGet(GLenum pname, GLfloat* params) {
        switch (pname) {
        case GL_MAJOR_VERSION:
            *params = g_cobalt.gl / 10;
            break;
...
#undef GO
    ...
    void COBALT_API cobalt_glGetIntegerv(GLenum pname, GLint* params) {
        DBG(COBALT_LOGD("glGetIntegerv(%s, %p)\n", PrintEnum(pname), params);)
        if (params == NULL) {
        ...
        noerrorShim();
        GLfloat fparam;
        if (cobalt_commonGet(pname, &fparam)) {
            params[0] = fparam;
            return;
        }
        gltexture_t* tex;
    ...
    void COBALT_API cobalt_glGetFloatv(GLenum pname, GLfloat* params) {
        DBG(COBALT_LOGD("glGetFloatv(%s, %p)\n", PrintEnum(pname), params);)
        LOAD_GLES(glGetFloatv);
        noerrorShim();
        if (cobalt_commonGet(pname, params)) {
            return;
        }
    ...
    void COBALT_API cobalt_glGetDoublev(GLenum pname, GLdouble* params) {
        DBG(COBALT_LOGD("glGetDoublev(%s, %p)\n", PrintEnum(pname), params);)
        GLfloat tmp[4 * 4];
        LOAD_GLES(glGetFloatv);
        noerrorShim();
        if (cobalt_commonGet(pname, tmp)) {
            *params = *tmp;
            return;
        }
        switch (pname) {
    ...
    void COBALT_API cobalt_glGetMaterialfv(GLenum face, GLenum pname, GLfloat* params) {
        DBG(COBALT_LOGD("glGetMaterialfv(%sn %s, %p)\n", PrintEnum(face), PrintEnum(pname), params);)
    ...
    void COBALT_API cobalt_glGetClipPlanef(GLenum plane, GLfloat* equation) {
        DBG(COBALT_LOGD("glGetClipPlanef(%s, %p)\n", PrintEnum(plane), equation);)
    ...
    const GLubyte* COBALT_API cobalt_glGetStringi(GLenum name, GLuint index) {
        DBG(COBALT_LOGD("glGetStringi(%s, %d)\n", PrintEnum(name), index);)
        BuildExtensionsList();
    ...
    void cobalt_glGetMinmaxParameteriv(GLenum target, GLenum pname, GLint* params) {
        DBG(COBALT_LOGD("unsupported glGetMinmaxParameteriv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(pname), params);)
        errorShim(GL_INVALID_VALUE);
    }
    AliasExport(void, glGetMinmaxParameteriv, , (GLenum target, GLenum pname, GLint* params));

    void cobalt_glGetMinmaxParameterfv(GLenum target, GLenum pname, GLfloat* params) {
        DBG(COBALT_LOGD("unsupported glGetMinmaxParameterfv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(pname), params);)
        errorShim(GL_INVALID_VALUE);
    }
