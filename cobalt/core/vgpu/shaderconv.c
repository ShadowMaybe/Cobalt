 ...
 * @param second_pass Whether cobalt is attempting to solve a linking issue
...
char * ConvertShaderConditionally(struct shader_s * shader_source, int second_pass){
    ...
    // ESSL 1.0 pipeline
    if(!hardext.glsl300es || g_cobalt.vgpu_backport) {
        if (g_cobalt.vgpu_dump){
        ...
        VerbosePrint(shader_source->converted, "Optimized shader (backward port) with cobalt post process");
    ...
    VerbosePrint(shader_source->converted, "Optimized shader with cobalt post process");
...
void VerbosePrint(char * source, char * stage) {
    if(g_cobalt.vgpu_dump > 1 && source != NULL){
        printf("Stage - %s :\n%s\n", stage, source);
    }
}
...
char * ConvertShaderMinimal(char * input, int is_fragment) {
    ...
    // even after the cobalt shaderconv pass, there still remain things that dont have a defined precision, like sampler2DShadow
    ...
    if(is_fragment) {
    	...
    	// remove the extension that cobalt shaderconv adds in when it detects frag data acccesses
    	input = cobalt_inplace_replace_simple(input, &shaderLength, "#extension GL_EXT_draw_buffers : enable", "");
    }else {
        // Gl4es post-process add old glsl keywords
    ...
    // cobalt only supports float attributes from the es 2.0 days
...
char * ConvertShaderMinimalBackport(char * input, size_t * length, int is_fragment, int destructive) {
    input = cobalt_inplace_replace_simple(input,  length, "flat ", "");
 ...
 * @param second_pass Whether cobalt tries to solve a linking error
 */
char * ConvertShaderVgpu(struct shader_s * shader_source, int second_pass){
    ...
    if(GetShaderVersion(source) == 100) {
        ...
        if((g_cobalt.vgpu_force_conv || g_cobalt.vgpu_backport) && second_pass){
            ...
            source = cobalt_inplace_replace_simple(source, &sourceLength, "flat ", "");
            source = cobalt_inplace_replace_simple(source, &sourceLength, "bvec", "vec");
            source = cobalt_inplace_replace_simple(source, &sourceLength, "ivec", "vec");
    ...
    // Avoid keyword clash with cobalt #define blocks
    //printf("REPLACING KEYWORDS");
    source = cobalt_inplace_replace_simple(source, &sourceLength, "#define texture2D texture\n", "");
    ...
    */
    ...
    source = cobalt_inplace_replace_simple(source, &sourceLength, "\"", "");
    ...
    source = cobalt_inplace_replace_simple(source, &sourceLength, "#define texture texture2D\n", "");
    source = cobalt_inplace_replace_simple(source, &sourceLength, "#define attribute in\n", "");
    source = cobalt_inplace_replace_simple(source, &sourceLength, "#define varying out\n", "");
...
char * InsertExtension(char * source, int * sourceLength, const int insertPoint, const char * extension){
    ...
    source = cobalt_inplace_replace_simple(source, sourceLength, "__EXT__", extension);
    return source;
}
...
char * ReplaceModOperator(char * source, int * sourceLength){
    ...
    for(int i=0;i<*sourceLength; ++i){
        ...
        replacementString = cobalt_inplace_replace(replacementString, &replacementSize, "x", leftOperand);
        replacementString = cobalt_inplace_replace(replacementString, &replacementSize, "y", rightOperand);
...
char * CoerceIntToFloat(char * source, int * sourceLength){
    ...
    source = cobalt_inplace_replace_simple(source, sourceLength, "ivec", "vec");
    source = cobalt_inplace_replace_simple(source, sourceLength, "uvec", "vec");

    source = cobalt_inplace_replace_simple(source, sourceLength, "isampleBuffer", "sampleBuffer");
    source = cobalt_inplace_replace_simple(source, sourceLength, "usampleBuffer", "sampleBuffer");

    source = cobalt_inplace_replace_simple(source, sourceLength, "isampler", "sampler");
    source = cobalt_inplace_replace_simple(source, sourceLength, "usampler", "sampler");
    ...
    source = cobalt_inplace_replace_simple(source, sourceLength, "gl_VertexID", "float(gl_VertexID)");
    source = cobalt_inplace_replace_simple(source, sourceLength, "gl_InstanceID", "float(gl_InstanceID)");
...
char * ForceIntegerArrayAccess(char* source, int * sourceLength){
    ...
    source = cobalt_inplace_replace_simple(source, sourceLength, "]", ")]");
    source = cobalt_inplace_replace_simple(source, sourceLength, "[", "[int(");

    // Step 3, restore all marked empty []
    source = cobalt_inplace_replace_simple(source, sourceLength, markerStart, "[");
    source = cobalt_inplace_replace_simple(source, sourceLength, markerEnd, "]");
...
char * ReplaceVariableName(char * source, int * sourceLength, char * initialName, char* newName, size_t offset) {
    ...
    for (int i = 0; i < strlen(charBefore); ++i) {
        for (int j = 0; j < strlen(charAfter); ++j) {
            ...
            int found = cobalt_countstring_simple(source + offset, toReplace);
            if(found){
                source = cobalt_resize_if_needed(source, sourceLength, found * strlen(replacement));

                // Pre reservation has been done, so there should be no pointer mutation
                char * new_source = cobalt_inplace_replace_simple(source + offset, sourceLength, toReplace, replacement);
...
char * ReplaceGLFragData(char * source, int * sourceLength){
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        source = cobalt_inplace_replace_simple(source, sourceLength, &needle[0], &replacement[0]);
...
char * ReplaceGLFragColor(char * source, int * sourceLength){
    if(strstr(source, "gl_FragColor")){
        ...
        source = cobalt_inplace_replace_simple(source, sourceLength, "gl_FragColor", "vgpu_FragData0");
    }
    return source;
}
...
/**
 * Replace and inserts precision qualifiers as necessary, see COBALT_VGPU_PRECISION
 * @param source The shader as a string
...
char * OverridePrecision(char * source, unsigned long * sourceLength) {
    if (g_cobalt.vgpu_precision != 0){
        ...
        switch (g_cobalt.vgpu_precision) {
